---
title: Text Fields Section
manager: soliver
ms.date: 11/16/2014
ms.audience: Developer
ms.topic: reference
f1_keywords:
- Vis_DSS.chm2155
localization_priority: Normal
ms.assetid: f743e4ee-343d-0edd-4a6d-5f104781a524
description: Отображает функции и формулы, вставленные в текст фигуры с помощью диалогового окна Field.
ms.openlocfilehash: ce6a658a553182e360eaa978254fc59b5bd5b59b
ms.sourcegitcommit: 8657170d071f9bcf680aba50b9c07f2a4fb82283
ms.translationtype: MT
ms.contentlocale: ru-RU
ms.lasthandoff: 04/28/2019
ms.locfileid: "33434951"
---
# <a name="text-fields-section"></a>Text Fields Section

Отображает функции и формулы, вставленные в текст фигуры с помощью диалогового окна **Field.** 
  
## <a name="remarks"></a>Примечания

Единственный способ добавить этот раздел — вставить поле в форму с помощью диалогового окна **Field** **(Вкладка Вставка).** 
  
Каждое текстовое поле в текстовом блоке имеет соответствующую строку в окне ShapeSheet. Имя строки представляет количество символов, которые содержит поле.
  

