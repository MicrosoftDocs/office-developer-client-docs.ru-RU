...
| _projname_opt_ <br/> |Необязательна  <br/> |**String** <br/> | Проект, содержащий макрос.  <br/> |
   
## <a name="remarks"></a>Примечания

Если указан проект, Microsoft Visio сканирует все открытые документы, содержащие _projname_opt_ и вызывает _макрос имяМакроса_ в этом проекте. Если параметр _projname_opt_ опущен или равен null (""), предполагается, что _имяМакроса_ находится в проекте VBA документа, содержащего оцениваемую формулу ЗапускМакроса. 
  
Функция RUNMACRO отличается от функции CALLTHIS в том, что она не передает ссылку на фигуру, которой принадлежит формула, оцениваемую до _имяМакроса_. Как и в CALLTHIS, для вызова функции ЗАПУСКМАКРОСА не требуется ссылаться на _projname_opt_ . 
