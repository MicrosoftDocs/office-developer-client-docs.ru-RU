...
Возвращает integer, от 1 до 7, представляющее будний день в  _дате_ или  _выражении_.
...
Результат соответствует с понедельника (1) по воскресенье (7). Округление не выполняется. Если  _время даты_ отсутствует или не может быть истолковано как допустимая дата или время, функция возвращает #VALUE! ошибка. 
  
Функция WEEKDAY также принимает одно значение  номера для выражения, в котором вся часть результата представляет количество дней с 30 декабря 1899 г. 
