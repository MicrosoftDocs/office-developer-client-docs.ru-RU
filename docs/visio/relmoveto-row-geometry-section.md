---
title: Строка RelMoveTo (раздел геометрии)
manager: soliver
ms.date: 03/09/2015
...
---
# <a name="relmoveto-row-geometry-section"></a>Строка RelMoveTo (раздел геометрии)
...
|[Y](y-cell-geometry-section.md) <br/> |Если строка **RelMoveTo** является первой строки в разделе, ячейки Y представляет *y* -координата первой вершины фигуры. Если отображается строка **RelMoveTo** между двумя строками, ячейки Y представляет *y* -координата первой вершины после приостановки выполнения в поле путь.  <br/> |
