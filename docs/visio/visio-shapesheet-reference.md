---
title: Справочник по таблицам свойств фигур Visio
manager: soliver
ms.date: 11/16/2014
ms.audience: Developer
ms.topic: reference
ms.assetid: ca06502c-884c-c1ad-d2c9-31f2b2b30ae5
description: Здесь представлены основные обзоры, задачи программирования, примеры и справочные материалы, которыми можно руководствоваться при разработке решений на основе Visio 2013.
localization_priority: Priority
ms.openlocfilehash: 56fa9ee3fa8b86eea08039b0064efed978735e3d
ms.sourcegitcommit: d6695c94415fa47952ee7961a69660abc0904434
ms.translationtype: HT
ms.contentlocale: ru-RU
ms.lasthandoff: 01/18/2019
ms.locfileid: "28726367"
---
# <a name="visio-shapesheet-reference"></a>Справочник по таблицам свойств фигур Visio

Здесь представлены основные обзоры, задачи программирования, примеры и справочные материалы, которыми можно руководствоваться при разработке решений на основе Visio 2013.
  
Мы ждем ваших комментариев и вопросов по этому содержимому. Вы можете отправить свой отзыв по адресу **[docthis@microsoft.com](mailto:docthis@microsoft.com)**. 
  
 **Первая публикация**: июль 2012 г. | **Обновлено**: октябрь 2012 г.
  
Эта документация включает следующие разделы:
  
- [Новые возможности](what-s-new-for-visio-shapesheet-developers.md). Содержит список новых элементов по объектам (в алфавитном порядке).
    
- [Понятия](concepts-visio-shapesheet.md). Содержит важные понятия для разработки пользовательских решений для таблиц свойств фигур Visio.
    
- [Распространенные задачи](common-tasks-visio-shapesheet.md). Приводятся некоторые короткие процедуры для наиболее распространенных задач таблиц свойств фигуры.
    
- [Справочник](reference-visio-shapesheet.md). Представлены справочные материалы для функций, ячеек, строк и разделов таблицы свойств фигуры.
    
## <a name="see-also"></a>См. также

- [Центр по разработке для Visio](https://msdn.microsoft.com/office/aa905478.aspx)    
- [Специальные возможности в продуктах Майкрософт](https://www.microsoft.com/enable/products/default.aspx)    
- [Обозначения в документации](https://msdn.microsoft.com/office/aa905365.aspx)   
- [Уведомление об авторских правах пакета SDK Visio](visio-sdk-copyright-notice.md)

    

