...
---
# <a name="cell-element-moveto-row-visio-xml"></a>Элемент Cell (строка MoveTo) (Visio XML)
...
|:-----|:-----|:-----|
|[Элемент Row (Geometry)](row-element-geometry-sectionvisio-xml.md) <br/> |[MoveTo_Type](moveto_type-complextypevisio-xml.md) <br/> |Содержит координаты X или Y первой вершины фигуры или представляет координаты X или Y первой вершины после разрыва пути.  <br/> |
...
|:-----|:-----|:-----|
|X  <br/> |Если **строка MoveTo** является первой строкой в разделе, **ячейка X** представляет x-координату первой вершины фигуры. Если **строка MoveTo** отображается между двумя строками, ячейка **X** представляет x-координату первой вершины после разрыва пути.  <br/> |[MoveTo Row (Geometry Section)](moveto-row-geometry-section.md) <br/> |
|Да  <br/> |Если **строка MoveTo** является первой строкой в разделе, ячейка Y представляет координату **Y** первой вершины фигуры. Если **строка MoveTo** отображается между двумя строками, ячейка Y представляет координату **Y** первой вершины после разрыва пути.  <br/> |[MoveTo Row (Geometry Section)](moveto-row-geometry-section.md) <br/> |
