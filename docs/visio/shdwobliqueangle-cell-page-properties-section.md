...
Содержит число, определяющие угол наклонного направления при применении типа теней страницы по умолчанию.
  
## <a name="remarks"></a>Примечания

Значение 0 (0) в этой ячейке указывает, что направление угла прямо вверх и измеряется перемещением по часовой стрелке.
  
 Угол, описанный в этой ячейке, используется всякий раз, когда для ячейки ShapeShdwType (типа теней для фигуры на странице) установлено значение Page Default **(visFSTPageDefault),** а тип тени является косым. Тип теней страницы по умолчанию определяется в ячейке ShdwType. 
  
Чтобы настроить это поведение для отдельной фигуры, используйте ячейку ShapeShdwObliqueAngle в разделе "Формат заливки".
