...
Указывает описательный запрос или комментарий для ячейки, определяемой пользователем. Приложение автоматически заключит быстрый текст в кавычках (""), чтобы указать, что это строка текста. Если введите равный знак (=) и опустить кавычка, можно ввести формулу в этой ячейке, которую оценивает приложение.
...
|||
|:-----|:-----|
| Имя ячейки:  <br/> | Пользователь.  *Имя*  . Подсказка, где пользователь.  *Имя*  — это имя строки  <br/> |
