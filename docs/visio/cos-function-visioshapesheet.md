---
title: COS Function (VisioShapeSheet)
manager: soliver
ms.date: 03/09/2015
ms.audience: Developer
ms.topic: reference
f1_keywords:
- Vis_DSS.chm82251407
localization_priority: Normal
ms.assetid: 2e6c82e7-a025-9df5-a95d-b00b0f39724f
description: Возвращает cosine угла.
ms.openlocfilehash: ee8e2f4a256617c37c4aac7876aa33ae04ab2f97
ms.sourcegitcommit: 8657170d071f9bcf680aba50b9c07f2a4fb82283
ms.translationtype: MT
ms.contentlocale: ru-RU
ms.lasthandoff: 04/28/2019
ms.locfileid: "33432739"
---
# <a name="cos-function-visioshapesheet"></a>COS Function (VisioShapeSheet)

Возвращает cosine угла. 
  
## <a name="syntax"></a>Синтаксис

COS *(угол)* 
  
### <a name="parameters"></a>Параметры

|**Имя**|**Необходимость**|**Тип данных**|**Описание**|
|:-----|:-----|:-----|:-----|
| _угол_ <br/> |Обязательный  <br/> |**Числовой** <br/> |Угол, под которым можно получить косин.  <br/> |
   
## <a name="remarks"></a>Примечания

Функция COS использует радианы, если не указать другую единицу измерения в *угол.* 
  
## <a name="example"></a>Пример

COS (45 дег) 
  
Возвращает 0.7071. 
  

