...
Представляет структуру [ACCT_BIN](acct_bin.md) , содержащую идентификатор учетной записи Exchange. 
...
Используйте [PROP_ACCT_IS_EXCH](prop_acct_is_exch.md) , чтобы проверить, является ли учетная запись учетной записью Exchange. Если это так, **MAPI_EMSMDB_UID\_PROP** — это **ACCT_BIN** , который содержит **емсмдбуид**, который является уникальным идентификатором для учетной записи Exchange. Если учетная запись не является учетной записью Exchange, это свойство не определено.
