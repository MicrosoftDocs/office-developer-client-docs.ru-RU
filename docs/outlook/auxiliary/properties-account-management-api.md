...
|**Свойство**|**Описание**|
|:-----|:-----|
|[PidTagNextSendAcct](pidtagnextsendacct.md) <br/> |Это вторичная отметка "отправить" для сообщения.  <br/> |
|[PidTagPrimarySendAccount](pidtagprimarysendaccount.md) <br/> |Это основная отметка "отправить" для сообщения.  <br/> |
...
|[PROP_ACCT_ID](prop_acct_id.md) <br/> |Возвращает идентификатор, который уникальным образом идентифицирует учетную запись в профиле, в котором создается учетная запись.  <br/> |
|[PROP_ACCT_IS_EXCH](prop_acct_is_exch.md) <br/> |True, если учетная запись является учетной записью Exchange.  <br/> |
|[PROP_ACCT_MINI_UID](prop_acct_mini_uid.md) <br/> |Возвращает уникальный идентификатор учетной записи в профилях Outlook.  <br/> |
...
|[PROP_ACCT_USER_DISPLAY_NAME](prop_acct_user_display_name.md) <br/> |Возвращает или задает отображаемую имя пользователя.  <br/> |
...
|[PROP_INET_SSL](prop_inet_ssl.md) <br/> |Указывает, следует ли использовать SSL для общего почтового ящика Интернета.  <br/> |
|[PROP_INET_USE_SPA](prop_inet_use_spa.md) <br/> |Указывает, следует ли использовать безопасную проверку подлинности паролей (SPA) для общего почтового ящика в Интернете.  <br/> |
...
|[PROP_MAPI_TRANSPORT_FLAGS](prop_mapi_transport_flags.md) <br/> |Представляет параметры транспорта, которые Outlook использует для определения необходимых задач синхронизации и отключения элементов пользовательского интерфейса, которые учетная запись не поддерживает.  <br/> |
|[PROP_POP_LEAVE_ON_SERVER](prop_pop_leave_on_server.md) <br/> |Указывает, что копия сообщения на сервере будет оставлена для учетной записи POP.  <br/> |
...
|[PROP_SMTP_USE_SPA](prop_smtp_use_spa.md) <br/> |Указывает, следует ли использовать безопасную проверку подлинности паролей (SPA) для учетной записи SMTP.  <br/> |
