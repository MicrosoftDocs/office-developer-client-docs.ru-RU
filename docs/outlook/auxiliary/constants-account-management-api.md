...
В этом разделе содержатся определения констант, идентификаторы классов и идентификаторы интерфейсов для API управления учетной записью.
...
|E_INVALIDARG  <br/> | *Как определено в файле загона winerror.h комплекта средств разработки программного обеспечения для Windows (SDK).*  <br/> |
|E_NOTIMPL  <br/> | *Как определено в файле загона Windows SDK winerror.h.*  <br/> |
...
|Следующие константы, ENCRYPT_, используются свойством [PROP_SMTP_SECURE_CONNECTION](prop_smtp_secure_connection.md) для указания типа зашифрованного подключения.  <br/> ||
...
|Следующие константы, SMTP_, используются свойством [](prop_smtp_auth_method.md) PROP_SMTP_AUTH_METHOD и указывают метод проверки подлинности.  <br/> ||
|SMTP_AUTH_SAME_AS_POP  <br/> |0  <br/> |
|SMTP_AUTH_RECEIVE_BEFORE_SEND  <br/> |2   <br/> |
|SMTP_AUTH_USER_PASS  <br/> |1   <br/> |
|Следующие 5 констант и макрос [](prop_pop_leave_on_server.md) используются свойством PROP_POP_LEAVE_ON_SERVER и указывают параметры для учетных записей POP, чтобы оставить копию сообщения на сервере.  <br/> ||
...
Используйте макрос DEFINE_GUID, определенный в файле guiddef.h файла header Windows SDK, чтобы связать символьное имя GUID с его значением.
