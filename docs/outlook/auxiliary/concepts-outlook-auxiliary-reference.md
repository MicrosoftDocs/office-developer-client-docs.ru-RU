---
title: Понятия (Outlook вспомогательные ссылки)
manager: soliver
ms.date: 11/16/2014
ms.audience: Developer
ms.topic: overview
localization_priority: Normal
ms.assetid: fa68a055-9e58-40ef-886e-4e8ebb08a2a9
description: В этом разделе описываются четыре API в вспомогательном справочнике Outlook 2013 г., а также примеры задач, которые используют эти API.
ms.openlocfilehash: c546166c42742fc54ee43eef7d006183b7c8a40e
ms.sourcegitcommit: 8657170d071f9bcf680aba50b9c07f2a4fb82283
ms.translationtype: MT
ms.contentlocale: ru-RU
ms.lasthandoff: 04/28/2019
ms.locfileid: "33417919"
---
# <a name="concepts-outlook-auxiliary-reference"></a>Понятия (Outlook вспомогательные ссылки)

В этом разделе описываются четыре API в вспомогательном справочнике Outlook 2013 г., а также примеры задач, которые используют эти API.

- [Управление учетными записями Outlook](outlook-account-management.md)
    
- [Сведения об API уровня ухудшения данных](about-the-data-degradation-layer-api.md)
    
- [Сведения об интерфейсах API, экспортируемых Outlook](about-apis-exported-by-outlook.md)
    
- [Конфигурация Outlook](outlook-configuration.md)
    
- [Календари и встречи Outlook](outlook-calendars-and-appointments.md)
    
- [Примеры задач](sample-tasks.md)
    

