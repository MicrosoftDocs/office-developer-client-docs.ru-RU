...
Указывает первичную отметку "отправить" учетную запись для сообщения.
...
Это свойство применяется к объекту сообщения MAPI. Для полученного сообщения на главной учетной записи указывается, с какой учетной записью должен быть отправлен форвард или ответ. Для исходятого сообщения определяется, с какой учетной записью отправлять сообщение. Его значение [PROP_ACCT_SEND_STAMP](prop_acct_send_stamp.md) [интерфейса IOlkAccount](iolkaccount.md) учетной записи, с которой отправляется сообщение. 
