...
Это свойство также определяет формат строки адреса в **PR_EMAIL_ADRESS** ([PidTagEmailAddress](pidtagemailaddress-canonical-property.md)). Строка, которую она предоставляет, может содержать только буквы в верхнем регистре от A до Z и цифры от 0 до 9.
