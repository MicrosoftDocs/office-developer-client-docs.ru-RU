...
|||
|:-----|:-----|
|Связанные свойства:  <br/> |ПР_ОБЖЕКТ_ТИПЕ  <br/> |
...
Тип объекта, содержащийся в этом свойстве, соответствует основному интерфейсу, доступному для объекта, доступного через интерфейс **OpenEntry** . Обычно он получается с помощью параметра _лпулобжтипе_ , возвращаемого соответствующим методом **OpenEntry** . Когда интерфейс получается другими способами, вызовите метод [IMAPIProp::](imapiprop-getprops.md) -Props, чтобы получить значение этого свойства. 
