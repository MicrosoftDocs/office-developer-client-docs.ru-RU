...
Проверяет правильность набора столбцов таблицы для использования поставщиком службы при последующем вызове метода [IMAPITable::SetColumns.](imapitable-setcolumns.md) 
...
Функция **FBadColumnSet** обрабатывает столбцы типа PT_ERROR как недопустимые, а столбцы типа PT_NULL как допустимые. 
