...
> Удаляет все строки из таблицы и всех соответствующих представлений, отправляя одно уведомление TABLE_RELOAD.
    
 _лпровсеттоделете_
  
> возврата Указатель на набор строк, который описывает удаляемые строки. Параметр _лпровсеттоделете_ может иметь значение null, если установлен флаг TAD_ALL_ROWS в параметре _ulFlags_ . 
