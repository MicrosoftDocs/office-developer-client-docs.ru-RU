...
Содержит строку, которая именует первый сервер, используемый для отправки сообщения.
...
Указывает первый сервер, который клиент должен использовать для отправки почты. Формат этих свойств зависит от реализации. Эти свойства могут использоваться клиентом для определения сервера, через который направляется почта, но являются необязательными и значение не имеет значения для сервера.
