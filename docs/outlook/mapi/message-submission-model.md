...
3. Поставщик транспорта получает доступ к данным сообщения с помощью передаваемого экземпляра **iMessage** . Для каждого получателя в переданном **iMessage** , для которого он принимает ответственность, поставщик транспорта задает свойство **PR_RESPONSIBILITY** ([PidTagResponsibility](pidtagresponsibility-canonical-property.md)), а затем возвращает.
