...
Список обязательных столбцов в таблице получения папок представлен в статье [получение таблиц с папками](receive-folder-tables.md). 
  
Реализуйте таблицы приема-передачи для поддержки настройки ограничений свойств для свойства **PR_RECORD_KEY** ([PidTagRecordKey](pidtagrecordkey-canonical-property.md)). Это позволяет легко получить доступ к определенным папкам получения.
