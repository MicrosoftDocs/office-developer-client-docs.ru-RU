...
1. Вызовите метод [iMessage:: жетреЦипиенттабле](imessage-getrecipienttable.md) , чтобы получить доступ к таблице получателей и идентификатору записи получателя, его свойство **PR_ENTRYID** ([PidTagEntryId](pidtagentryid-canonical-property.md)).
...
3. Вызовите метод [IMAPIProp::-PROPS](imapiprop-getprops.md) получателя для получения свойства **PR_SEND_RICH_INFO** ([PidTagSendRichInfo](pidtagsendrichinfo-canonical-property.md)). Если для **PR_SEND_RICH_INFO** ЗАДАНО значение true, получатель может обрабатывать форматированный текст. 
