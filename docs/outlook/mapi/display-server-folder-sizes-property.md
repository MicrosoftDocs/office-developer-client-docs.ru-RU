...
Чтобы запросить размер папки на сервере, Outlook открывает папку в хранилище с [IMsgStore:: OpenEntry](imsgstore-openentry.md), передавая флаг **мапи_но_каче**, а затем отправляет запрос на **пр_мессаже_сизе_екстендед**. Поставщик услуг хранения должен возвратить размер папки на сервере.
  
Чтобы запросить размер папки на локальном диске, Outlook открывает папку без флага **мапи_но_каче** . Затем он запрашивает **пр_мессаже_сизе_екстендед**; поставщик хранилища должен возвратить размер указанной папки на локальном диске.
