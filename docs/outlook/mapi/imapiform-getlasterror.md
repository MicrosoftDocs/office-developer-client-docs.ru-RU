...
> Если задано, строки в структуре **мапиеррор** , возвращаемые в параметре _лппмапиеррор_ , имеют формат Юникод. Если флаг МАПИ_УНИКОДЕ не установлен, строки представлены в формате ANSI. 
...
Структуру **мапиеррор** , на которую указывает параметр _лппмапиеррор_ , можно использовать, только если параметр **GetLastError** возвращает значение S_OK. Иногда MAPI не может определить, какая Последняя ошибка, или больше не содержит ничего больше об ошибке. В этом случае указатель на NULL возвращается в _лппмапиеррор_ . 
