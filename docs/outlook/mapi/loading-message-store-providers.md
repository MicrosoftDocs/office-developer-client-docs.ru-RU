...
6. Если клиент не установил флаг МАПИ_НО_МАИЛ при входе в систему и не задал МДБ_НО_МАИЛ в действии 1, MAPI передает идентификатор записи в буфер обмена MAPI, чтобы диспетчер очереди MAPI мог войти в хранилище сообщений.
...
> [!NOTE]
> Если при входе в систему поставщика хранилища сообщений возникает ошибка, так как был введен неправильный пароль, а поставщик хранилища сообщений не может отобразить интерфейс для запроса правильного пароля, он должен возвратить МАПИ_Е_ФАИЛОНЕПРОВИДЕР из **функцииimsprovider:: logon **метод. Это позволит клиентам запрашивать пароль для повторного входа в систему поставщика хранилища сообщений вместо того, чтобы запретить поставщику использовать MAPI для всего сеанса. 
