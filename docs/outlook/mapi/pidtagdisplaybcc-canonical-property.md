...
Если у сообщения нет получателей скрытой копии, то хранилище сообщений должно отвечать на вызов [IMAPIProp::::/PROPS](imapiprop-getprops.md) со значением возврата S_OK и пустой строкой для **PR_DISPLAY_BCC**. 
...
- Точка с запятой должна быть символом, который используется для разделения имен в свойствах **PR_DISPLAY_BCC**, **PR_DISPLAY_CC** ([PidTagDisplayCc](pidtagdisplaycc-canonical-property.md)) и **PR_DISPLAY_TO** ([PidTagDisplayTo](pidtagdisplayto-canonical-property.md)). Точки с запятой не допускаются в именах получателей в MAPI. 
