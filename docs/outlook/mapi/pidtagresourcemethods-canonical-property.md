...
Содержит битмаску флагов, которые указывают методы в **интерфейсе IMAPIStatus,** поддерживаемые объектом состояния. 
...
Это свойство указывает, какие из методов в реализации **объекта состояния IMAPIStatus** поддерживаются. Объектам status разрешено возвращать MAPI_E_NO_SUPPORT из неподтверченных методов. 
  
Клиенты используют свойство PR_RESOURCE_METHODS **объекта** состояния, чтобы не звонить на неподтверченные методы. Если заданной является флаг, соответствующий определенному методу, метод существует и может быть вызван. Если этот флаг понятен, метод не следует называть. 
