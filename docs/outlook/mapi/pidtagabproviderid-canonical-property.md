...
Поставщик адресных книг может предоставить несколько идентификаторов. Например, поставщик, который предоставляет два разных контейнера, может публиковать в **PR_AB_PROVIDER_ID** уникальные идентификаторы для каждого контейнера. 
  
 **PR_AB_PROVIDER_ID** является аналогом свойства **PR_MDB_PROVIDER** ([PidTagStoreProvider](pidtagstoreprovider-canonical-property.md)) для хранилищ сообщений. Клиентские приложения могут использовать **PR_AB_PROVIDER_ID** для поиска связанных строк в таблице иерархий адресной книги. 
