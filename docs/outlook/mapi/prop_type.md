...
Макрос **PROP_TYPE** для определения типа свойства. Например, вызов PROP_TYPE **(PR_ENTRYID** [(PidTagEntryId))](pidtagentryid-canonical-property.md)приводит к PT_BINARY возвращается.
  
Каждый тег свойства содержит тип свойства в слове низкого порядка (биты от 0 до 15) и идентификатор свойства в слове высокого порядка (биты от 16 до 31). Макрос **PROP_TYPE** извлекает тип свойства и помещает его в биты от 0 до 15 возвращаемого integer. Остальные биты возвращаемого значения устанавливаются до нуля. 
