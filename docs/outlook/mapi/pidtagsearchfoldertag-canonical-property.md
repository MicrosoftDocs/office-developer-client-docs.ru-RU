...
Содержит значение, используемное для синхронизации этого сообщения определения с контейнером папки поиска.
...
Это свойство меняется при смене сообщения определения. Она должна изменять каждую итерацию, но может быть не уникальной.
