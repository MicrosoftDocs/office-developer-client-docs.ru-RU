...
|||
|:-----|:-----|
|Связанные свойства:  <br/> |ПР_СЕНД_ИНТЕРНЕТ_ЕНКОДИНГ  <br/> |
...
> Кодирует текст сообщения в формате HTML. Этот флаг игнорируется, если не установлен флаг ЕНКОДИНГ_МИМЕ. 
    
БОДИ_ЕНКОДИНГ_ТЕКСТ_АНД_ХТМЛ 
  
> Кодирование текста сообщения с помощью текстовых и HTML-файлов в виде многоцелевого альтернативного расширения почты в Интернете (MIME). Этот флаг игнорируется, если не установлен флаг ЕНКОДИНГ_МИМЕ. 
...
МАК_АТТАЧ_ЕНКОДИНГ_АППЛЕДАУБЛЕ 
  
> Кодирование вложений Macintosh в двойном режиме Apple. Этот флаг игнорируется, если не установлен флаг ЕНКОДИНГ_МИМЕ. 
    
МАК_АТТАЧ_ЕНКОДИНГ_АППЛЕСИНГЛЕ 
  
> Кодирование вложений Macintosh в Apple Single Mode. Этот флаг игнорируется, если не установлен флаг ЕНКОДИНГ_МИМЕ. 
    
МАК_АТТАЧ_ЕНКОДИНГ_УУЕНКОДЕ 
  
> Кодирование вложений Macintosh в КОДИРОВКе UUENCODE. Если установлен флаг ЕНКОДИНГ_МИМЕ, этот флаг игнорируется, а вместо этого используется кодировка BinHex. 
