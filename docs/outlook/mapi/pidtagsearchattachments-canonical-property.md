...
Содержит строку в Юникоде, запрашиваемую в содержимом вложения в хранилище.
  
## 

|||
|:-----|:-----|
|Связанные свойства:  <br/> |ПР_СЕАРЧ_АТТАЧМЕНТС_В  <br/> |
...
> [!NOTE]
> Этот тег ограничения MAPI, используемый при поиске содержимого вложений, может не определяться в файле заголовков, доступных для загрузки. Вы можете добавить его в код с помощью следующего значения: _Гт_`#define PR_SEARCH_ATTACHMENTS_W PROP_TAG(PT_UNICODE, 0x0EA5)`
