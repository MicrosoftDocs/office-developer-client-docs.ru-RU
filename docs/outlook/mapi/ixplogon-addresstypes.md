...
При выборе поставщика транспорта, который должен обрабатывать получателей исходящих сообщений, диспетчер очереди MAPI использует каждую строку параметра _лпппсзадртипеаррай_ для проверки сравнения. Если свойство **PR_ADDRTYPE** получателя сообщения ([PidTagAddressType](pidtagaddresstype-canonical-property.md)) в точности совпадает со строкой, определяющей один из типов адресов обмена сообщениями, предоставленных поставщиком транспорта, поставщик может доставить сообщение этому получателю.
