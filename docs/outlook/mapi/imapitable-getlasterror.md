...
Можно использовать структуру [мапиеррор](mapierror.md) , на которую указывает параметр _лппмапиеррор_ , если объект Table предоставляет один, только если **GetLastError** возвращает S_OK. Иногда реализация таблицы не может определить, в чем последняя ошибка, или больше не может сообщить об ошибке. В этом случае указатель по адресу _лппмапиеррор_ имеет значение null. 
