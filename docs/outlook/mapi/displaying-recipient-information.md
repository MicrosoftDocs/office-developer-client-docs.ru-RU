...
Существует два метода вызова этого диалогового окна: [IAddrBook::D етаилс](iaddrbook-details.md) и [имаписуппорт::D етаилс](imapisupport-details.md). Когда поставщик вызывает один из этих методов для запроса сведений о получателе, MAPI сначала открывает получателя, вызывая метод [IMAPIContainer:: OpenEntry](imapicontainer-openentry.md) контейнера. Затем вызывается метод получателя [IMAPIProp:: опенпроперти](imapiprop-openproperty.md) , чтобы запросить свойство **PR_DETAILS_TABLE** ([PidTagDetailsTable](pidtagdetailstable-canonical-property.md)). **PR_DETAILS_TABLE** — это свойство, представляющее таблицу отображения сведений о получателях. 
...
1. Прежде чем пользователь получит доступ к элементу управления, вызовите метод [ипропдата:: хрсетобжакцесс](ipropdata-hrsetobjaccess.md) , чтобы задать для элемента управления доступ к IPROP_CLEAN. 
...
4. Если уровень доступа IPROP_DIRTY, пользователь изменил элемент управления. Поставщик должен выполнять следующие действия:
    
   - Call [ипропдата:: хрсетпропакцесс](ipropdata-hrsetpropaccess.md) , чтобы вернуть уровень доступа в значение IPROP_CLEAN. 
...
5. Если уровень доступа все еще IPROP_CLEAN, элемент управления не был изменен. 
