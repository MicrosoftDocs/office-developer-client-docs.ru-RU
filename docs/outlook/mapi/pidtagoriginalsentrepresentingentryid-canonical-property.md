...
Содержит идентификатор записи пользователя обмена сообщениями, от имени которого было отправлено исходное сообщение.
...
Клиентские приложения, отправляющие сообщение от имени другого клиента, должны установить для этого свойства значение свойства **PR_SENT_REPRESENTING_ENTRYID** ([PidTagSentRepresentingEntryId)](pidtagsentrepresentingentryid-canonical-property.md)при первой отправке сообщения. После этого его не следует менять.
