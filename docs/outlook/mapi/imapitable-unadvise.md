...
Используйте метод **IMAPITable::** unadvise, чтобы освободить указатель на объект приемника уведомлений, переданный в параметре _лпадвисесинк_ при предыдущем вызове метода **IMAPITable:: Advise**, тем самым отменяя регистрацию уведомлений. При отмене указателя на объект приемника уведомлений вызывается метод " **IUnknown:: Release** " объекта. Как правило, **выпуск** вызывается во время вызова метода **unadvise** , но если другой поток находится в процессе вызова метода [имапиадвисесинк:: OnNotify](imapiadvisesink-onnotify.md) для приемника уведомлений, вызов **освобождения** задерживается до возвращения метода **OnNotify** . 
