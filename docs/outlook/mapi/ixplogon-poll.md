...
Диспетчер очереди MAPI периодически вызывает метод **иксплогон::P олл** , если поставщик транспорта указывает, что он должен опрашиваться на наличие новых сообщений, которые поставщик выполняет, передав флаг LOGON_SP_POLL в вызов метода [Иксппровидер:: транспортлогон](ixpprovider-transportlogon.md) в начале сеанса. Если поставщик транспорта указывает в ответ на вызов **опроса** , для обработки которого доступно одно или несколько входящих сообщений, то диспетчер очереди MAPI вызывает метод [Иксплогон:: стартмессаже](ixplogon-startmessage.md) , который позволяет поставщику обработать первое входящее сообщение. Поставщик транспорта указывает на входящие сообщения, присвоив параметру _лпулинкоминг_ значение ненулевого значения. 
