...
В приведенном ниже примере кода показано, как создать ограничение, которое отфильтровывает все сообщения, которые не содержат слово "воллэйбалл" в строке темы и не были отправлены в Сью из диспетчера учетных записей. Дерево структуры [срестриктион](srestriction.md) является обязательным, **а** верхний узел является ограничением, реализованным с помощью структуры [сандрестриктион](sandrestriction.md) . Три ограничения, соединяемые операцией **and** , — это ограничение для вложенного объекта, которое выполняет поиск сообщений, отправленных в Сью, ограничение на содержимое, которое выполняет поиск сообщений **** от Sam, а также другие ограничения, которые ищут сообщения с темой, содержащей слово "воллэйбалл". Так как **пр_субжект** ([PidTagSubject](pidtagsubject-canonical-property.md)) не является обязательным свойством, необходимо включить ограничение **Exists** . 
  
В этом коде используется динамическое выделение и инициализация; также можно выделять и инициализировать статически. В интересах краткости проверка ошибок, которая должна выполняться после вызова выделения, не включена в пример. 
