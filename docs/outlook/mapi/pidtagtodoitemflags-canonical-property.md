...
Представляет состояние To-Do помеченного элемента.
...
Это свойство является битным полем, в котором каждый бит должен быть установлен до 1, если применяется связанное условие в следующей таблице, в противном случае 0.
...
|8   <br/> |todoRecipientFlagged  <br/> |Следует установить только объект черновика сообщения, а это означает, что объект помечен для получателей.  <br/> |
   
Все биты, не указанные в таблице, зарезервированы. Они должны быть проигнорированы, но должны быть сохранены, если они установлены.
