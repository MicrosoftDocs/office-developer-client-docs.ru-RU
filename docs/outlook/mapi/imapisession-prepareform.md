...
> возврата Указатель на идентификатор интерфейса (IID), представляющий интерфейс, который будет использоваться для доступа к сообщению. Передача **значений NULL** в стандартный интерфейс (или [iMessage](imessageimapiprop.md)) используется. Параметр _лпинтерфаце_ должен иметь **значение NULL** или IID_IMessage. 
