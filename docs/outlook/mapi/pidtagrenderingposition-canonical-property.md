...
Если указанное смещение равно-1 (0xFFFFFFFF), то вложение не отображается с помощью этого свойства. Все значения, отличные от-1, указывают положение в свойстве **PR_BODY** ([PidTagBody](pidtagbody-canonical-property.md)), в котором будет отображаться вложение.
  
 **Note (Примечание** ) Символ, указанный этим свойством в **PR_BODY** , заменяется вложением. Как правило, этот символ является пробелом, хотя также можно использовать специальный замещающий символ. 
