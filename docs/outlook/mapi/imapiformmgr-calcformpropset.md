...
Если средство просмотра формы передает MAPI_UNICODEный флаг в параметре _ulFlags_ , все строки должны быть возвращены в виде строк Юникода. Поставщики библиотеки форм, не поддерживающие строки Юникода, должны возвращать MAPI_E_BAD_CHARWIDTH, если MAPI_UNICODE передается. 
