...
Чтобы задать уровень доступа к свойству, сначала необходимо вызвать **хрсетобжакцесс** с флагом IPROP_READWRITE, установленным в параметре _улакцесс_ , чтобы сделать объект изменяемым. Затем вызовите метод [ипропдата:: хрсетпропакцесс](ipropdata-hrsetpropaccess.md) , указав свойство Target в массиве, на которое указывает параметр _лппроптагаррай_ . 
