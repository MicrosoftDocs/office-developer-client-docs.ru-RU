...
Эти свойства обычно содержат имя электронной почты получателя, то есть конечный компонент адреса электронной почты, который уникальным образом идентифицирует получателя в локальной организации. Имя электронной почты соответствует различаемого имени X.400, которое должно быть коротким именем, гарантированно уникальным в пределах определенного домена обмена сообщениями.
