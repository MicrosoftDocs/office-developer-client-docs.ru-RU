...
Содержит отображаемую имя получателя в безопасной форме, которую нельзя изменить.
...
Эти свойства должны быть реализованы всеми поставщиками адресных книг. Они содержат версию отображаемого имени получателя, передаваемого вместе с сообщением. Для большинства поставщиков адресных книг эти свойства имеют то же значение, что и **PR_DISPLAY_NAME** ([PidTagDisplayName).](pidtagdisplayname-canonical-property.md) Поставщики, не защищенные отображаемой именем, PT_ERROR MAPI изменяет отображаемую именем путем добавления кавычка вокруг имени.
  
Клиентские приложения могут использовать это свойство для предотвращения изменения или "спуфинга" записей. Примером спуфинга является передача John Doe as John (What a Guy) Doe.
