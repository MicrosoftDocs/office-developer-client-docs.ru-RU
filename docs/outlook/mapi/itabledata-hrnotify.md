...
Метод **ITableData::HrNotify** отправляет уведомление TABLE_ROW_MODIFIED строке, которая соответствует строке, описанной свойствами, на которые указывает параметр _lpSPropValue._ **HrNotify** отправляет уведомление независимо от того, были ли внесены изменения в строку. Все клиенты и поставщики услуг, которые имеют представления таблицы и вызвали [IMAPITable::Advise](imapitable-advise.md) для регистрации уведомлений в своих представлениях, получают это уведомление. 
