...
Для реализации надстройки автономного состояния необходимо реализовать функции подключения, инициализации и других функций настройки. В этом разделе эти функции подключения, инициализации и установки демонстрируются с помощью примеров кода из надстройки Sample Offline State. Пример надстройки, позволяющей управлять автономным состоянием, — это надстройка COM, добавляющее **автономное состояние** в меню Outlook и использует API автономного режима. В меню **автономного состояния** можно включить или отключить мониторинг состояния, проверить текущее состояние и изменить текущее состояние. Дополнительные сведения о скачивании и установке надстройки, позволяющей управлять автономным состоянием, см. в статье [Установка надстройки, позволяющей управлять автономным состоянием](installing-the-sample-offline-state-add-in.md). Дополнительные сведения об API автономного режима см. в статье [Об API автономного режима](about-the-offline-state-api.md).
  
После настройка надстройки автономного состояния необходимо реализовать функции мониторинга и изменения изменений состояния подключения. Дополнительные сведения см. в [статью Мониторинг изменений состояния подключения с помощью надстройки автономного состояния.](monitoring-connection-state-changes-using-an-offline-state-add-in.md)
  
## <a name="on-connection-routine"></a>О режиме подключения

Метод **[IDTExtensibility2.OnConnection](https://msdn.microsoft.com/library/extensibility.idtextensibility2.onconnection%28v=VS.80%29.aspx)** вызван при каждом загрузке надстройки. Это точка входа для надстройки, поэтому код, который вы вложили в функцию, будет вызван после начала  `OnConnection` надстройки. В следующем примере функция  `OnConnection` вызывает  `HrInitAddin` функцию. 
...
Функция вызывает и выполняет функции, чтобы завершить настройку надстройки  `HrInitAddin`  `LoadLibraries`  `HrCacheProfileName`  `HrAddMenuItems` автономного состояния. 
...
## <a name="load-libraries-routine"></a>Режим загрузки библиотек

Функция загружает файлы библиотеки динамических ссылок  `LoadLibraries` (DLL), которые требуются надстройку. 
...
## <a name="cache-profile-name-routine"></a>Обычное имя профиля кэша

Функция  `HrCacheProfileName` вызывает **[функцию IMAPISupport::OpenProfileSection,](imapisupport-openprofilesection.md)** чтобы открыть раздел профилей для текущего сеанса, а затем задает профиль для обработчиков кнопок. 
...
## <a name="add-menu-items-routine"></a>Добавление обычных элементов меню

Функция определяет параметры меню, которые отображаются в меню состояние автономного режима, которое создается при загрузке надстройки в Outlook, а затем вызывает каждый `HrAddMenuItems` элемент  `DispEventAdvise` меню. 
