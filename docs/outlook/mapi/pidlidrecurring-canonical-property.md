...
Данное свойство указывает, представляет ли объект повторяющиеся ряды. Значение TRUE указывает, что объект представляет ряд повторяющихся данных. Значение FALSE или отсутствие этого свойства указывает на то, что объект представляет либо один экземпляр, либо исключение (в том числе потерянный экземпляр). Обратите внимание на разницу между этим свойством и свойством **LID_IS_RECURRING** ([PidLidIsRecurring](pidlidisrecurring-canonical-property.md)).
