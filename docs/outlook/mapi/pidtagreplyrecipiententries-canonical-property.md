...
Содержит массив идентификаторов записей размером для получателей, которые должны получить ответ.
...
Если этого свойства нет, ответ отправляется только пользователю, который определен свойством **PR_SENDER_ENTRYID** ([PidTagSenderEntryId).](pidtagsenderentryid-canonical-property.md) Когда это и свойства **PR_REPLY_RECIPIENT_NAMES** ([PidTagReplyRecipientNames),](pidtagreplyrecipientnames-canonical-property.md)ответ отправляется всем получателям, определенным этими двумя свойствами. Поставщик транспорта использует эти свойства для переопределения обычной логики ответа.
  
Если установлено либо это свойство, либо **PR_REPLY_RECIPIENT_NAMES,** необходимо также установить другое свойство. Эти свойства должны содержать одинаковое количество получателей, и они должны содержать их в одном порядке. Несоблюдение этих требований может привести к непредсказуемым результатам. 
