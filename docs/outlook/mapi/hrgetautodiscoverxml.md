...
- Клиентский компьютер не подключен к сети, клиентский компьютер не подключен к серверу Microsoft Exchange 2007, _пвзаддресс_ не является учетной записью на сервере Exchange 2007, или _пвзаддресс_ — это учетная запись, которая не поддерживает Exchange Служба автоматического обнаружения. 
    
МАПИ_Е_УСЕР_КАНЦЕЛ 
  
- Для отмены операции в _хканцелевент_ был передан дескриптор события. 
    
СТРСАФЕ_Е_ИНСУФФИЦИЕНТ_БУФФЕР
