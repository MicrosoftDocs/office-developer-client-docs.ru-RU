...
> возврата Число, которое показывает текущий уровень выполнения (рассчитывается из параметров _улкаунт_ и _ултотал_ или из параметров _лпулмин_ и _лпулмакс_ метода [IMAPIProgress:: SetLimits](imapiprogress-setlimits.md) ) между глобальной Нижний предел и глобальное верхнее предельное значение. 
...
Не все три параметра **IMAPIProgress: требуются:P рогресс** . Единственный обязательный параметр — _улвалуе_, число, указывающее процент выполнения. Если установлен флаг МАПИ_ТОП_ЛЕВЕЛ, можно также передать Счетчик объектов и общий объект. Некоторые реализации используют эти значения для отображения фразы, например "5 элементов, завершенных из 10" с индикатором хода выполнения. 
