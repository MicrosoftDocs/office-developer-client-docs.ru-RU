...
Это свойство используется для запроса корреляции входящих отчетов с исходным отправленным сообщением. Когда поставщик транспорта сталкивается с  отправленным сообщением, для PR_CORRELATE установлено PR_CORRELATE_MTSID **(** [PidTagCorrelateMtsid)](pidtagcorrelatemtsid-canonical-property.md)идентификатора системы передачи сообщений (MTS) для этого сообщения.
  
 **PR_CORRELATE** системы обмена сообщениями, поддерживаюющие корреляцию идентификатором MTS, например X.400. 
