...
Чтобы разрешить класс сообщения в форму, передайте имя разрешенного класса сообщения (например, `IPM.HelpDesk.Software`). Чтобы обеспечить точное разрешение (то есть, чтобы предотвратить разрешение для базового класса класса Message), можно передать флаг MAPIFORM_EXACTMATCH в параметре _ulFlags_ . 
