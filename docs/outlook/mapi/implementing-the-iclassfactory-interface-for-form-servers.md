...
[IClassFactory](https://msdn.microsoft.com/library/ms694364%28VS.85%29.aspx) — это интерфейс OLE, который клиентские приложения используют для создания новых объектов формы класса сообщений сервера формы. В следующей таблице перечислены необходимые методы **IClassFactory.** 
  
|**Способ**|**Описание**|
|:-----|:-----|
|[CreateInstance](https://msdn.microsoft.com/library/ms682215%28v=VS.85%29.aspx) <br/> |Создает новый объект формы.  <br/> |
|[LockServer](https://msdn.microsoft.com/library/ms682332%28v=VS.85%29.aspx) <br/> |Блокирует сервер формы в памяти, чтобы избежать накладных расходов при запуске нескольких объектов формы.  <br/> |
