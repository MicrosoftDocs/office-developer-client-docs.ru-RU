...
Указывает, связан ли объект с повторяющимся набором.
  
|||
|:-----|:-----|
|Связанные свойства:  <br/> |ЛИД_ИС_РЕКУРРИНГ  <br/> |
...
Значение TRUE указывает, что объект представляет либо повторяющийся ряд, либо исключение (включая потерянный экземпляр). Значение FALSE или отсутствие этого свойства указывает на то, что объект представляет один экземпляр. Обратите внимание на разницу между этим свойством и свойством **пр_рекурринг** ([PidLidRecurring](pidlidrecurring-canonical-property.md)).
