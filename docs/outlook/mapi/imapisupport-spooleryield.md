...
Если **спулериелд** ВОЗВРАЩАЕТ с мапи_в_канцел_мессаже, то диспетчер очереди MAPI обнаружил, что сообщение больше не должно отправляться. ВозВращайте МАПИ_Е_УСЕР_КАНЦЕЛ к вызывающему процессу и выйдите из него, если это возможно. 
