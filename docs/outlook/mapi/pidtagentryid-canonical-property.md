...
Это свойство всегда доступно с помощью метода [IMAPIProp::-PROPS](imapiprop-getprops.md) , следующего за первым вызовом метода [IMAPIProp:: SaveChanges](imapiprop-savechanges.md) . Некоторые поставщики услуг могут сделать их доступными сразу же после создания экземпляра. Поставщик должен всегда возвращать долгосрочный идентификатор записи из **PROPS**. Таким образом, чтобы преобразовать краткосрочный идентификатор в долгосрочный, просто откройте объект и получите его свойство через **PROPS**. 
  
В следующей таблице приведены важные различия между этим свойством, **PR_RECORD_KEY** ([PidTagRecordKey](pidtagrecordkey-canonical-property.md)) и **PR_SEARCH_KEY** ([PidTagSearchKey](pidtagsearchkey-canonical-property.md)). 
