...
В этом разделе содержится пример в C++, `HrOpenContact`который показывает, как использовать структуру [CONTAB_ENTRYID](contab_entryid.md) , определяющую запись в адресной книге Contacts, чтобы получить связанное Контактное сообщение MAPI. 
