...
> возврата Указатель на GUID, который можно использовать для уникальной идентификации этого объекта из других автономных объектов. Он должен быть **гуид_глобалстате**.
...
При использовании [GetProcAddress](https://msdn.microsoft.com/library/ms683212.aspx) для поиска адреса этой функции в MSMapi32. dll укажите **хропеноффлинеобж @ 20** в качестве имени процедуры. 
  
 **Хропеноффлинеобж** работает только для клиентов, которые являются поставщиками MAPI, надстройками COM и клиентскими расширениями Exchange, запущенными в процессе Outlook. В противном случае **хропеноффлинеобж** возвращает **мапи_е_нот_фаунд**. 
