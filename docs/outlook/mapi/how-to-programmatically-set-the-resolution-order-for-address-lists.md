...
В этом разделе содержится пример кода в C++, который программным образом задает порядок списков адресов, с помощью которых реципиенты в сообщениях электронной почты и участники в запросах собраний решаются.
  
В MAPI каждый профиль может поддерживать несколько списков адресов, и каждый список адресов находится в своем контейнере. MAPI поддерживает метод **[SetSearchPath](https://support.microsoft.com/kb/292590)** в интерфейсе, который позволяет установить новый путь поиска в профиле, который используется для разрешения имен. Чтобы использовать **метод IAddrBook::SetSearchPath,** необходимо определить нужный порядок разрешения в массиве **[SRowSet,](srowset.md)** который содержит контейнеры соответствующих адресных книг в нужном порядке, а затем указать массив в качестве *параметра lpSearchPath.* Первым свойством для каждой записи в массиве **SRowSet** должно быть PR_ENTRYID соответствующей адресной книги. **[](pidtagentryid-canonical-property.md)** 
  
Пример кода задает порядок разрешения в следующих действиях:
...
3.  Войдите в MAPI и позволяет пользователю выбрать профиль. 
...
9. Ищет входные ИД нужных списков адресов, сравнивая имена нужных списков адресов с существующими именами в иерархии  `ANROrder` адресных книг. 
    
10. Задает соответствующие ID-записи для **массива SRowSet.** `pNewRows`
    
11. Вызовы и передачи в качестве  `pNewRows`  *параметра lpSearchPath*  в **IAddrBook::SetSearchPath** для набора пути поиска. 
...
В этом примере кода используются списки адресов, доступные в установке **Microsoft Office Outlook:** Все контакты, **все** группы и **контакты.** Пример необходимо выполнить после Outlook запуска и запуска в инициализированном профиле. Пример хорошо работает с именами, которые находятся на одном языке (например, все имена на английском языке). Она не предназначена для работы в многоязычных развертываниях, например в папке **Contacts,** локализованной для пользователя, запускаемого не на английском языке Outlook сборки. 
...
- [О настройке порядка разрешения списков адресов в Outlook](about-setting-the-resolution-order-for-address-lists-in-outlook.md)
