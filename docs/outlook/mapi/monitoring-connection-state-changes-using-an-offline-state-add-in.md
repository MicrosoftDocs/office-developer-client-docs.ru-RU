...
`InitMonitor` Функция вызывает функцию **хропеноффлинеобж** . `InitMonitor` Функция вызывает **Кмйоффлиненотифи** , чтобы Outlook мог отправлять уведомления обратного вызова клиенту и зарегистрировал обратный вызов с помощью переменной `AdviseInfo` **[MAPIOFFLINE_ADVISEINFO](mapioffline_adviseinfo.md)** .
