...
Сведения о создании проектов в Visual Studio с помощью примеров кода на основе ASMX, включенных в справочные разделы по интерфейсу Project Server (PSI).
...
Новые примеры PSI в SDK Project 2013 соответствуют формату, использующему службы Windows Communication Foundation (WCF). Основанные на ASMX примеры также можно адаптировать для использования служб WCF. В данной статье рассказывается, как использовать данные примеры с веб-службами ASMX. Сведения об использовании примеров со службами WCF см. в предварительных условия для примеров кода на основе [WCF в Project.](prerequisites-for-wcf-based-code-samples-in-project.md)
  
> [!NOTE]
> Интерфейс веб-службы ASMX PSI признан нерекомендуемым в Project Server 2013, однако все еще поддерживается. Если клиентская объектная модель (CSOM) включает методы, необходимые для вашего приложения, то новые приложения следует разрабатывать с использованием этой CSOM. CSOM позволяет приложениям работать с Project Online или локальной установкой Project Server 2013. В противном случае, если ваше приложение использует интерфейс PSI, то оно должно использовать интерфейс WCF, что является рекомендуемой технологией для сетевого взаимодействия. Приложения, которые используют интерфейс ASMX или интерфейс WCF, могут работать только для локальной установки Project Server 2013. Дополнительные сведения об CSOM см. в архитектуре [Project Server 2013](project-server-2013-architecture.md) и клиентской объектной модели [(CSOM) для Project 2013.](client-side-object-model-csom-for-project-2013.md) 
...
3. **Установите файлы IntelliSense.**
    ...
    IntelliSense описания для веб-служб PSI требуют создания сборки прокси PSI с помощью скрипта CompileASMXProxyAssembly.cmd в подкамере загрузки project  `Documentation\IntelliSense\WSDL` 2013 SDK. Этот скрипт создает основанную на ASMX сборку прокси-сервера ProjectServerServices.dll. Дополнительные сведения см. в файле [ReadMe_IntelliSense] в загружаемом пакете SDK. 
...
4. **Задайте пространство имен для примера приложения**.
   ...
   Например, пример кода для [QueueRenameProject](https://msdn.microsoft.com/library/WebSvcProject.Project.QueueRenameProject.aspx) имеет пространство имен **Microsoft.SDK.Project.Samples.RenameProject.** Если имя проекта Visual Studio имеет значение **RenameProject**, скопируйте пространство имен из файла Program.cs, а затем откройте область **Свойства** проекта (в меню **Проект** выберите **Свойства RenameProject**). На вкладке **Приложение** скопируйте пространство имен в текстовое поле **Пространство имен по умолчанию**. 
...
5. **Задайте веб-ссылки**.
   ...
   - Создайте сборку прокси PSI с именем ProjectServerServices.dll, а затем установите ссылку на сборку. Чтобы получить IntelliSense, рекомендуется добавить ссылку на PSI. См. описание использования сборки прокси [PSI и IntelliSense.](#pj15_PrerequisitesASMX_BuildingProxy)
...
Сборку прокси-сервера ProjectServerServices.dll для всех веб-служб на основе ASMX в PSI можно создать и использовать с помощью скрипта CompileASMXProxyAssembly.cmd в папке загрузки project  `Documentation\IntelliSense\WSDL` 2013 SDK. Ссылку на загрузку см. в документации для разработчиков [Project 2013.](project-2013-developer-documentation.md)
  
> [!NOTE]
> При извлечении исходных файлов прокси-сервера из файла Source.zip файлы в папке актуальны на дату публикации загрузки  `Documentation\IntelliSense\WSDL\Source` SDK Project 2013. Чтобы создать обновленные исходные файлы прокси PSI, запустите скрипт GenASMXProxyAssembly.cmd на компьютере Project Server. Сценарии в  `Documentation\IntelliSense\WCF` папке не работают для приложений на основе ASMX. Скрипт GenWCFProxyAssembly.cmd вызывает SvcUtil.exe, который создает файлы исходного кода для служб WCF. Файлы прокси WCF включают в себя различные атрибуты, интерфейс канала и класс клиента для каждой службы PSI. Например, основанная на WCF служба Resource включает в себя интерфейс **ResourceChannel**, интерфейс **Resource** и класс **ResourceClient**. Основанная на ASMX веб-служба Resource включает в себя класс **Resource** с немного другими свойствами. 
...
Загрузка project 2013 SDK включает исходные файлы, созданные командой Wsdl.exe сборки прокси. Исходные файлы находятся в Source.zip в  `Documentation\IntelliSense\ASMX` подкафакторе. Вместо задания ссылки на сборку прокси-сервера вы можете добавить один или несколько этих исходный файлов в решение Visual Studio. Например, после выполнения скрипта GenASMXProxyAssembly.cmd добавьте файл wsdl.Project.cs в решение. Вместо запуска скрипта вы можете выполнить следующие команды для создания одного исходного файла, например: 
...
Если вы не используете основанную на ASMX сборку прокси-сервера и не добавляете выходной WSDL-файл, вы можете задать одну или несколько индивидуальных веб-ссылок. Ниже покажите, как установить веб-ссылку с помощью Visual Studio 2012.
...
4. В **текстовом** поле URL-адреса введите `https:// _ServerName_/ _ProjectServerName_/_vti_bin/psi/ _ServiceName_.asmx?wsdl` и нажмите кнопку **ВВОД** или нажмите значок **"Перейти".** Если установлен протокол SSL, вам следует использовать вместо протокола HTTP протокол HTTPS. 

   Например, используйте следующий URL-адрес для службы Project на сайте  `https://MyServer/pwa` для Project Web App: `https://MyServer/pwa/_vti_bin/psi/project.asmx?wsdl`
...
5. После разрешения ссылки введите имя ссылки в текстовое поле **"Имя** веб-ссылки". В примерах кода в документации разработчика Project 2013 используется произвольное стандартное имя **ссылки Svc _ServiceName._** Например, веб-служба Project называется **SvcProject** (см. рис. 3). 
    
   **Рис. 3. Добавление ссылки на веб-службу ASMX**

   ![Добавление ссылки на веб-службу ASMX Добавление]ссылки на(media/pj15_PrerequisitesASMX_AddWebSvcReference.gif "веб-службу ASMX")
  
Для компонентов приложения, которые должны выполняться на компьютере с Project Server, используйте олицетворение или установите более высокий уровень разрешений, используйте ссылку на службу WCF вместо веб-ссылки ASMX. Дополнительные сведения см. в предварительных условия для примеров кода на основе [WCF в Project.](prerequisites-for-wcf-based-code-samples-in-project.md)
...
Проверка подлинности пользователей локального сервера Project Server ( проверка подлинности Windows или проверка подлинности на основе форм) происходит с помощью обработки утверждений в SharePoint Server 2013. Многократная проверка подлинности означает, что веб-приложение, в котором Project Web App, поддерживает как проверку подлинности Windows, так и проверку подлинности на основе форм. Если имеет место указанная ситуация, вызов в веб-службу ASMX, которая использует проверку подлинности Windows, завершится со следующей ошибкой, так как процесс утверждений не может определить, для какого типа пользователей выполнять проверку подлинности:
...
Для приложений, основанных на WCF, используется другое решение. Дополнительные сведения см. в разделе "Использование нескольких способов проверки подлинности" в разделе "Предварительные условия для примеров кода на основе [WCF" в Project.](prerequisites-for-wcf-based-code-samples-in-project.md) 
...
В большинстве примеров используется одна или несколько переменных, которые необходимо обновить, чтобы пример работал в текущей среде соответствующим образом. В следующем примере при наличии SSL используйте протокол HTTPS вместо протокола HTTP. Замените  _имя сервера_ именем используемого сервера. Замените  _ProjectServerName_ на имя виртуального каталога сайта Project Server, например PWA. 
