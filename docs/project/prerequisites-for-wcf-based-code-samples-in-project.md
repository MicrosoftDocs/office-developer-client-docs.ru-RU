...
- Если файл ProjectServerServices.xml добавляется в тот же каталог, в котором установлена ссылка на сборку прокси ProjectServerServices.dll, то можно получить описания IntelliSense для классов и членов PSI. Для получения дополнительных сведений обратитесь к файлу [ReadMe_IntelliSense] в `Documentation\IntelliSense` папке загрузки пакета SDK для Project 2013. 
    
**Рис. 2. Использование IntelliSense для метода в службе Resource**

![Использование IntelliSense для метода реадресаурце](media/pj15_PrerequisitesWCF_Intellisense.gif "с использованием IntelliSense для метода реадресаурце")
...
4. После того как будет разрешена ссылка на службу, введите имя ссылки в текстовое поле **Пространство имен**. В примерах кода в документации для разработчиков Project 2013 используется произвольное имя пространства имен **svc _ServiceName_**. Например, служба Resource в примерах кода называется **SvcResource**.
