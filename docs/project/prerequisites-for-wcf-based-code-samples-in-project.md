...
Сведения, которые помогут вам создавать проекты в Visual Studio с помощью примеров кода на основе WCF, включенных в справочные разделы по интерфейсу Project Server (PSI).
   
Многие из примеров кода на основе WCF, включенных в [библиотеку классов Project Server 2013 и ссылку на веб-службу](https://msdn.microsoft.com/library/ef1830e0-3c9a-4f98-aa0a-5556c298e7d1%28Office.15%29.aspx) , были первоначально созданы для документации разработчика Project 2010 и используют стандартный формат для веб-служб WCF. Примеры по-прежнему работают в Project Server 2013 и предназначены для копирования в консольное приложение и запускаются как готовые единицы. Исключения из этого правила отмечаются в примере. 
  
Примеры кода в документации для разработчиков Project 2013, которые не изменяются из примеров, разработанных для Office Project Server 2007, используют веб-службы ASMX. Примеры на основе ASMX можно также адаптировать для использования служб WCF. В данной статье показывается, как использовать эти примеры со службами WCF. Сведения о том, как использовать примеры с веб-службами ASMX, можно найти [в разделе предварительНые требования для примеров кода на основе ASMX в Project](prerequisites-for-asmx-based-code-samples-in-project.md).
...
>
> Для получения дополнительных сведений о CSOM, ознакомьтесь с [архитектурОй Project Server 2013](project-server-2013-architecture.md) и [клиентской объектной моделью (CSOM) для Project 2013](client-side-object-model-csom-for-project-2013.md). 
...
3. **Установите файлы IntelliSense.**
    ...
    Для описаний IntelliSense для служб PSI необходимо создать сборку прокси-сервера PSI с помощью сценария Компилевкфпроксяссембли. cmd в `Documentation\IntelliSense\WCF` подкаталоге ЗАГРУЖАЕМОГО пакета SDK для Project 2013. Этот скрипт создает сборку прокси ProjectServerServices.dll на основе WCF. Дополнительные сведения см. в файле [ReadMe_IntelliSense].mht в загрузке пакета SDK. 
...
2. **Добавьте ссылки, необходимые для WCF.**
    ...
    В обозревателе решений добавьте ссылку на **System. ServiceModel** (см. рисунок 1). Веб-приложение использует **System. ServiceModel. Web**.
...
4. **Установите пространство имен для примера приложения.**
    ...
    Например, пример кода для [реадресаурце](https://msdn.microsoft.com/library/WebSvcResource.Resource.ReadResource.aspx) имеет пространство имен **Microsoft. SDK. Project. Samples. креатересаурцетест**. Если проект Visual Studio имеет имя **ResourceTest**, скопируйте пространство имен из файла Program.cs, а затем откройте панель **Свойства** (выберите в меню **Проект** пункт **Свойства ResourceTest**). На вкладке **Приложение** вставьте скопированное пространство имен в текстовое поле **Пространство имен по умолчанию**. 
...
Сборку прокси можно использовать для всех общедоступных служб WCF в PSI. Скомпилируйте сборку прокси-сервера ProjectServerServices. dll, используя `Documentation\IntelliSense\WCF\CompileWCFProxyAssembly.cmd` скрипт из ЗАГРУЖАЕМОГО пакета SDK для Project 2013, а затем скопируйте сборку прокси на компьютер разработчика. Скопируйте файл ProjectServerServices.xml для IntelliSense в то же расположение. В Visual Studio установите ссылку на сборку прокси ProjectServerServices.dll. 
  
Для пакетов обновления и обновлений Project Server можно обновить исходные файлы прокси и создать новую сборку прокси с помощью скрипта GenWCFProxyAssembly.cmd в той же самой папке загрузки пакета SDK. Ссылку на загружаемый пакет SDK можно найти в [документации для разработчиков Project 2013](project-2013-developer-documentation.md). Дополнительные сведения см. в разделе [Добавление ссылки на службу](#pj15_PrerequisitesWCF_AddingServiceReference). 
  
> [!NOTE]
> При извлечении исходных файлов прокси-сервера из исходного zip-файла файлы в `Documentation\IntelliSense\WCF\Source` папке будут считаться датой публикации ЗАГРУЖАЕМОГО пакета SDK для Project 2013. Чтобы создать обновленные исходные файлы прокси PSI, запустите скрипт GenASMXProxyAssembly.cmd на компьютере Project Server. Дополнительные сведения см. в разделе [Добавление ссылки на службу](#pj15_PrerequisitesWCF_AddingServiceReference). 
> 
...
- Если файл ProjectServerServices.xml добавляется в тот же каталог, в котором установлена ссылка на сборку прокси ProjectServerServices.dll, то можно получить описания IntelliSense для классов и членов PSI. Для получения дополнительных сведений см файл [Реадме_интеллисенсе] в `Documentation\IntelliSense` папке загрузки пакета SDK для Project 2013. 
    
**Рис. 2. Использование IntelliSense для метода в службе Resource**

![Использование IntelliSense для метода реадресаурце] (media/pj15_PrerequisitesWCF_Intellisense.gif "Использование IntelliSense для метода реадресаурце")
...
ПАКЕТ SDK для Project 2013 содержит исходные файлы, которые создаются с помощью команды SvcUtil. exe для сборки прокси-сервера. Исходные файлы находятся в исходном ZIP-файле в `Documentation\IntelliSense\WCF` подкаталоге. Вместо установки ссылки на сборку прокси можно добавить один или несколько исходных файлов в решение Visual Studio. Например, для использования службы Project и службы Resource добавьте в решение файлы wcf.Project.cs и wcf.Resource.cs. 
...
Если не используется сборка прокси на основе WCF и не добавляется файл прокси для службы PSI, можно установить отдельные ссылки на службы непосредственно в Visual Studio. Вы также можете использовать шаг 1 следующей процедуры для создания обновленных прокси-файлов, чтобы подготовиться к `Documentation\IntelliSense\WCF\GenWCFProxyAssembly.cmd` сценарию, включенному в загрузку пакета SDK для Project 2013. 
...
В следующей процедуре показано, как задать ссылку на службу с помощью Visual Studio 2012 на компьютере, на котором выполняется тестовая установка Project Server:
...
3. В диалоговом окне **Добавление ссылки на службу** в текстовом поле **адрес** введите https://localhost:32843/ _GUID_/ПСИ/ _ServiceName_. svc и нажмите клавишу **Ввод**. Замените _GUID_ именем виртуального каталога приложения службы Project Server, например 534c37eb00d74ccfadcecf9827e95239. Замените _ServiceName_ именем службы, например ресурсом (см. рис. 3). 
    
   Имя виртуального каталога службы Project Server можно получить одним из следующих способов.
    
   - Откройте приложение центра администрирования SharePoint 2013 в браузере. Выберите элемент **Управление приложениями-службами**, а затем выберите нужное приложение-службу PSI Project Server. Например, выберите **ProjectServerService**. URL-адрес страницы "Управление сайтами Project Web App" содержит имя виртуального каталога. Например, `https://ServerName:8080/_admin/pwa/managepwa.aspx?appid=534c37eb-00d7-4ccf-adce-cf9827e95239`имя виртуального каталога `534c37eb00d74ccfadcecf9827e95239` (имя каталога не содержит тире). 
   ...
   URL-адреса `https://localhost:32843/534c37eb00d74ccfadcecf9827e95239/PSI/Resource.svc` , такие как стандартные для служб Project Server. 
    
4. После того как будет разрешена ссылка на службу, введите имя ссылки в текстовое поле **Пространство имен**. В примерах кода в документации для разработчиков Project 2013 используется произвольное имя пространства имен **svc __**. Например, служба Resource в примерах кода называется **SvcResource**.
...
5. Замените временный файл Web. config в каталоге службы проекта исходным (переименованный в Web. config) и повторите попытку `iisreset`.
...
Если приложение настраивает службы WCF программными средствами, то оно не использует файл конфигурации службы. В противном случае приложение Windows или консольное приложение использует элемент **System. ServiceModel** в файле App. config; веб-приложение включает **System. ServiceModel** в файл Web. config. Для получения дополнительных сведений об использовании файла App. config или программной настройки служб WCF, ознакомьтесь со статьей поШаговое [руководство: Разработка приложений PSI с использованием WCF](https://msdn.microsoft.com/library/65707234-c3da-44e4-8364-32a6be28f645%28Office.15%29.aspx).
  
При создании исходного файла прокси службы команда SvcUtil.exe также создает файл output.config, который является основой для элемента **system.serviceModel** по умолчанию в файле app.config или web.config. Загрузка пакета SDK для Project 2013 включает в `Documentation\IntelliSense\WCF\Source.zip`себя пример файла Output. config. Например, файл output.config по умолчанию, создаваемый командой SvcUtil.exe для службы Resource, включает две привязки с именами **BasicHttpBinding_Resource** и **BasicHttpBinding_Resource1**. Элемент **client** включает две конечные точки по умолчанию. Одна конечная точка предназначена для безопасного доступа к HTTP-адресу в порте 32843, а другая — для обычного доступа в порте 32843, как показано ниже. 
...
3. Замените `ServerName/ProjectServerName` адрес конечной точки клиента на имя вашего сервера и экземпляра Project Web App. 
    
4. Замените `ServiceName` именем службы PSI, например ресурсом. Убедитесь, что заменили все три экземпляра имени службы, например:
...
Файл App. config можно изменить с помощью **редактора конфигурации службы WCF** в Visual Studio (меню " **Сервис** "). На рисунке 4 показано, как задать элемент " **контракт** " в диалоговом окне " **Редактор конфигурации службы Майкрософт** ". Если решение использует сборку прокси-сервера PSI, откройте ProjectServerServices. dll в `bin\debug` каталоге решения Visual Studio. В диалоговом окне **Обозреватель типов контрактов** отображаются все контракты службы WCF (см. рисунок 5). 
  
**Рис. 4. Использование редактора конфигураций службы WCF**

![Использование редактора конфигурации службы WCF] (media/pj15_PrerequisitesWCF_ServiceConfigurationEditor.gif "Использование редактора конфигурации службы WCF")
  
Если решение использует прокси-файл службы, например wcfResource.cs, скомпилируйте приложение и откройте исполняемый файл в `bin\debug` каталоге. Дополнительные сведения о редактировании файла app.config см. в пошаговом руководстве [Разработка приложений PSI с использованием WCF](https://msdn.microsoft.com/library/65707234-c3da-44e4-8364-32a6be28f645%28Office.15%29.aspx).
...
В следующем примере метод **DisableFormsAuth** может вызываться из каждого раздела **OperationContextScope** в приложении. Метод удаляет все значения заголовка, которые ранее отключили проверку подлинности на формах, поэтому проверка подлинности на формах может быть продолжена, если параметр _исвиндовсаус_ имеет **значение false**. Если _исвиндовсаус_ имеет **значение true**, метод **дисаблеформсаус** отключает проверку подлинности с помощью форм. 
...
> 
> Решение для приложения на основе ASMX отличается. Для получения дополнительных сведений ознакомьтесь с разделом *Использование нескольких проверок* подлинности в разделе [необходимые условия для примеров кода на основе ASMX в Project](prerequisites-for-asmx-based-code-samples-in-project.md). 
...
- Используйте страницу "Параметры сервера" в Project Web App `https://ServerName/ProjectServerName/_layouts/15/pwa/admin/admin.aspx`(), чтобы управлять всеми заданиями очереди и удалять или принудительно возвращать корпоративные объекты. Для доступа к этим ссылкам на странице параметров сервера требуются административные разрешения.
...
После тестирования некоторых примеров кода остаются корпоративные объекты и параметры, которые следует удалить или сбросить. С помощью страницы "Параметры сервера" в Project Web App можно управлять корпоративными данными `https://ServerName/ProjectServerName/_layouts/15/pwa/admin/admin.aspx`(). С помощью ссылок на странице параметров сервера можно удалять старые элементы, принудительно возвращать проекты, управлять очередью заданий для всех пользователей и выполнять другие административные задачи.
