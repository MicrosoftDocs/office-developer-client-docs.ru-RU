...
![Сквозной](media/8c0741f9-7f76-409d-8c00-e7a8c3ddb89f.png "Рабочий") процесс сквозного рабочего процесса
...
1. Необязательное. Сохраните полный URL-адрес проекта в переменной, которую можно использовать в рабочем процессе.
    
    ![Сохраните URL-адрес проекта в переменной].(media/a880c5c6-8e7a-44dd-87e9-7e532169d489.png "Сохраните URL-адрес проекта в переменной") .
  
2. Добавьте действие **дождаться события проекта** в рабочий процесс и выберите **при возврате проекта** событие. 
    
    ![Дождитесь, пока проект будет возвращен в](media/699aa9c7-b3c9-426e-a775-96993a13559c.png "ожидание возврата проекта")
...
3. Создайте словарь **рекуессеадер** с помощью действия " **Построение словаря** ". Вы будете использовать один и тот же заголовок запроса для всех вызовов веб-служб в этом рабочем процессе. 
    ...
    ![Создание словаря рекуессеадер](media/83b0aa10-9ab7-43dd-800d-a738bb815876.png "Построение словаря рекуессеадер")
...
4. Добавьте в словарь следующие два элемента:
    ...
    ![Добавление заголовка Accept с](media/2f2e2016-3c49-4cac-b1e7-f2b8118b840c.png "добавлением заголовка Accept")
...
5. Создайте словарь **рекуестбоди** с помощью действия " **Построение словаря** ". В этом словаре хранятся все обновления полей, которые требуется применить. 
    ...
    - **__metadata/Type** Тип метаданных поля. Эта запись всегда одинакова и использует следующие значения: 
       ...
       - Name: Кустомфиелддиктионари (i)/__metadata/Type (где **i** это индекс каждого настраиваемого поля в словаре, начиная с 0) 
            
       - Тип: строка
            
       - Значение: SP. Ключзначение
    
       ![Определение обновления настраиваемого поля],(media/a4423493-6603-42ee-ae50-1ef74c5c59bd.png "определяющего обновление настраиваемого поля")
    ...
    - **ValueType** Тип обновляемого настраиваемого поля. 
       ...
       |Имя|Тип|Значение|
       |:-----|:-----|:-----|
       |Кустомфиелддиктионари (0)/__metadata/Type  <br/> |String  <br/> |Состав. Ключзначение  <br/> |
       |Кустомфиелддиктионари (0)/Кэй  <br/> |String  <br/> |Настраиваемые\_ce23fbf43fa0e411941000155d3c8201  <br/> |
       |Кустомфиелддиктионари (0)/Валуе  <br/> |String  <br/> |Запись\_b9a2fd69279de411940f00155d3c8201; #Entry\_baa2fd69279de411940f00155d3c8201  <br/> |
       |Кустомфиелддиктионари (0)/Валуетипе  <br/> |String  <br/> |Edm.String  <br/> |
       |Кустомфиелддиктионари (1)/__metadata/Type  <br/> |String  <br/> |Состав. Ключзначение  <br/> |
       |Кустомфиелддиктионари (1)/Кэй  <br/> |String  <br/> |Custom_c7f114c97098e411940f00155d3c8201  <br/> |
       |Кустомфиелддиктионари (1)/Валуе  <br/> |String  <br/> |90,5  <br/> |
       |Кустомфиелддиктионари (1)/Валуетипе  <br/> |String  <br/> |Edm.Double  <br/> |
       |Кустомфиелддиктионари (2)/__metadata/Type  <br/> |String  <br/> |Состав. Ключзначение  <br/> |
       |Кустомфиелддиктионари (2)/Кэй  <br/> |String  <br/> |Custom_c6fb67e0b9a1e411941000155d3c8201  <br/> |
       |Кустомфиелддиктионари (2)/Валуе  <br/> |String  <br/> |2015 – 04 – 01T00:00:00.0000000  <br/> |
       |Кустомфиелддиктионари (2)/Валуетипе  <br/> |String  <br/> |Edm.DateTime  <br/> |
   
       ![Словарь, определяющий словарь обновлений настраиваемых полей](media/41a1f18f-a6b2-40ff-904b-437baf962621.png ", определяющий обновление настраиваемых полей")
  
6. Добавьте действие **Call HTTP Web Service** , чтобы вернуть проект. 
    
    ![Вызов метода Checkout](media/8ce56014-0317-419b-afa7-229d05c86885.png "метод") Checkout
...
8. Добавьте действие **Call HTTP Web Service** , чтобы вызвать метод **упдатекустомфиелдс** . 
    ...
    ![Вызовите метод упдатекустомфиелдс](media/03b323f1-8e99-4b18-be18-be505d7cec7e.png ", вызовите метод упдатекустомфиелдс")
...
9. Измените свойства вызова веб-службы, чтобы присоединить параметры **рекуессеадер** и **рекуестконтент** к созданным словарям. Кроме того, можно создать новую переменную для хранения **респонсеконтент**.
    ...
    ![Привязка словарей к заголовку запроса и контенту](media/f96bec92-138e-4eab-b1e7-1ab83d0428a5.png "привязывать словари к заголовку и содержимому запроса")
  
10. Необязательное. Прочтите словарь ответа, чтобы проверить состояние задания в очереди и записать сведения в список журнала рабочего процесса.
    
    ![Настройка ведения]журнала Настройка(media/7d2f4936-61d7-4906-83e8-7478a5935af5.png "ведения") журнала
...
11. Добавьте вызов веб-службы в конечную точку **публикации** , чтобы опубликовать проект. Всегда используйте один и тот же заголовок запроса. 
    ...
    ![Вызовите метод Publish](media/3b661091-ffae-4d7e-a0bb-5b96a6292731.png ", вызовите метод Publish")
  
    ![Свойства для вызова веб-службы публикации](media/6a80a5d3-7e29-4398-993c-f78b3faca8b1.png "для вызова веб-службы публикации")
  
12. Добавьте конечный вызов веб-службы в конечную точку **возврата** , чтобы проверить проект в. 
    
    ![Вызов метода checkin](media/430510cb-0774-4911-af7f-b565b83eba0e.png "вызовите метод checkin")
  
    ![Свойства для свойства вызова веб-службы возврата](media/485f48d6-bbb8-4568-9dc3-aae3218f6bd1.png "для вызова веб-службы возврата")
...
**Необходимые компоненты:** Прежде чем можно будет использовать **CreateProjectSite**, необходимо задать параметр **Разрешить пользователям выбирать** для создания сайта проекта в **параметрах PWA** > * * подключенные сайты SharePoint * * > **Параметры**.
  
![Настройка параметра "разрешить пользователям выбирать" в](media/6c6c8175-eb10-431d-8056-cea55718fdb4.png "параметрах PWA разрешить пользователям выбирать параметры PWA")
...
4. Добавьте действие **Call HTTP Web Service** . Измените тип запроса, чтобы он использовал **POST**, и задайте URL-адрес в следующем формате:
    ...
    ![Построение URI конечной точки CreateProjectSite](media/42a90a5e-8d1b-4667-a933-785175212847.png ", который создает URI конечной точки CreateProjectSite")
...
5. Измените свойства вызова веб-службы, чтобы присоединить параметр **рекуессеадер** к созданному вами словарю. 
    ...
    ![Привязка словаря к запросу](media/61a5a0a8-405f-44eb-b5e7-80b11f7caec3.png "привязки словаря к запросу")
