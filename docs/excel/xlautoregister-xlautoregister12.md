...
Ваша реализация **xlAutoRegister** должна выполнять поиск с учетом регистра, используя внутренние списки функций и команд, которые она экспортирует, в поисках сравнения с переданным именем. Если функция или команда найдена, **xlAutoRegister** должна попытаться зарегистрировать ее, используя функцию **xlfRegister** , убедившись, что вы задаете строку, сообщающую Excel о типах возвращаемых данных и аргументах функции, а также о любой другой обязательной информации о функции. Затем он должен вернуться в Excel, что бы возвращался вызов **xlfRegister** . Если функция была зарегистрирована успешно, **xlfRegister** возвращает значение **КСЛТИПЕНУМ** , содержащее идентификатор регистра функции. 
