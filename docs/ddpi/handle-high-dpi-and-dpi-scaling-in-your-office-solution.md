...
|Поддержка определения DPI на уровне системы |  Приложения определяют разрешение основного подключенного монитора при входе в Windows, но не могут реагировать на изменение разрешения. Дополнительные сведения см. в разделе "Настройка [Windows для](#configure-windows-to-fix-blurry-apps) исправления размытых приложений" этой статьи.  | Приложение — растровое изображение, растягиваемое при перемещении на новый дисплей с другим разрешением.    |
...
При создании новых дочерних окон убедитесь, что их поддержка определения DPI соответствует родительскому окну. Функцию [GetWindowDpiAwarenessContext](https://docs.microsoft.com/windows/desktop/api/winuser/nf-winuser-getwindowdpiawarenesscontext) можно использовать для получения информации о DPI родительского окна. Дополнительные сведения о согласовании поддержки определения DPI см. в разделе "Принудительный сброс поддержки определения DPI на уровне процесса" статьи [Разработка классических приложений с высоким разрешением для Windows](https://docs.microsoft.com/windows/desktop/hidpi/high-dpi-desktop-application-development-on-windows#related-topics).
...
- [Разработка приложения Per-Monitor DPI-Aware WPF](https://docs.microsoft.com/windows/desktop/hidpi/declaring-managed-apps-dpi-aware) предоставляет общий обзор и руководство по написанию классических приложений Win32. Многие из методов, описанных в этой статье применяются для решений по расширению Office.
- 
