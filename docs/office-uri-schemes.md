...
Допустимые URI должны соответствовать стандартным стандартам идентификаторов ресурсов (IRI) RFC 3987-International. Символы, определенные как зарезервированные в RFC 3986, не должны быть закодированы в процентах. . Имена файлов не должны содержать следующие символы: \/:? \< \> | "или \* .  
...
Обратите внимание, что вертикальная черта, используемая как разделитель в этой спецификации, не входит в число символов, определенных в разделе 2.2 документа RFC 3986 как зарезервированные для возможного использования в качестве разделителей. Это сделано специально, чтобы расширить число символов, поддерживаемых аргументами команд URI без их кодирования с использованием символа процента. В \<command-argument\> сегментах зарезервированные символы RFC 3986 ":" и "/" входят в данные аргумента, а не разделители, поэтому они включаются в escape-последовательность.
...
Схема ms-infopath определяет синтаксис URI для открытия или создания документа Microsoft Infopath. В ней определены две команды, которые указывают, что необходимо сделать с указанным документом. Команды представлены в виде 1) Open-for-Edit-cmd (OFE), который указывает InfoPath на открытие документа по указанному универсальному коду ресурса (URI) для редактирования; и 2) Open-for-View-cmd (OFV), который указывает InfoPath на открытие документа в заданном URI в режиме только для чтения.
