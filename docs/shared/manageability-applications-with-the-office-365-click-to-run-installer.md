...
- <span data-ttu-id="400f6-126">**Загрузка**: файлы для загрузки нового контента на клиенте для установки более поздней версии.</span><span class="sxs-lookup"><span data-stu-id="400f6-126">**Download**: Downloads new content to the client for later installation.</span></span>
...
-  <span data-ttu-id="400f6-203">_contentid_: Определяет содержимое для загрузки с сервера через настраиваемый диспетчер бит.</span><span class="sxs-lookup"><span data-stu-id="400f6-203">_contentid_: Identifies the content to download from the content server through the customized BITS manager.</span></span> <span data-ttu-id="400f6-204">Это значение передается через интерфейс БИТОВ для интерпретации.</span><span class="sxs-lookup"><span data-stu-id="400f6-204">This value is passed through the BITS interface for interpretation.</span></span>
...
- <span data-ttu-id="400f6-219">Это действие может быть запущено, только если состояние COM одно из следующих:</span><span class="sxs-lookup"><span data-stu-id="400f6-219">This action can only be triggered when the COM status is one of the following:</span></span> 
  ...
  - <span data-ttu-id="400f6-225">**eAPPLY_FAILED**</span><span class="sxs-lookup"><span data-stu-id="400f6-225">**eAPPLY_FAILED**</span></span>
...
### <a name="summary-of-iupdatenotify2-interface"></a><span data-ttu-id="400f6-253">Сводка по IUpdateNotify2 интерфейс</span><span class="sxs-lookup"><span data-stu-id="400f6-253">Summary of IUpdateNotify2 interface</span></span>
...
- <span data-ttu-id="400f6-267">Если вы хотите использовать новые методы, необходимо убедитесь, что:</span><span class="sxs-lookup"><span data-stu-id="400f6-267">If you want to use the new methods, you need to make sure:</span></span>
...
- <span data-ttu-id="400f6-282">Для `Addfile` и `AddFileWithRanges` — это URL-адрес удаленной функции, в следующем формате:</span><span class="sxs-lookup"><span data-stu-id="400f6-282">For the  `Addfile` and  `AddFileWithRanges` functions, the remote URL is in the following format:</span></span> 
  ...
  -  <span data-ttu-id="400f6-285">_ \<относительный путь к файлу целевой\> _ предоставляет расположение и имя файла для загрузки.</span><span class="sxs-lookup"><span data-stu-id="400f6-285">_\<relative path to target file\>_ provides the location and file name of the file to download.</span></span> 
...
<span data-ttu-id="400f6-292">**На следующем рисунке приведено Общие сведения о загрузке образа**</span><span class="sxs-lookup"><span data-stu-id="400f6-292">**The following image is an overview of downloading a custom image**</span></span>
...
- <span data-ttu-id="400f6-312">Замените *% версии* построения версии Office.</span><span class="sxs-lookup"><span data-stu-id="400f6-312">Replace  *%version%*  with the build version of Office.</span></span> <span data-ttu-id="400f6-313">Могут быть получены из клиентское обновление метаданных (описано в следующем разделе).</span><span class="sxs-lookup"><span data-stu-id="400f6-313">This can be derived from the Client Update metadata (explained in the next section).</span></span> 
...
5. <span data-ttu-id="400f6-323">Создайте образ локальной сети CDN, перебирая список файлов XML и копирования файлов CDN, при создании структуру папок при необходимости.</span><span class="sxs-lookup"><span data-stu-id="400f6-323">Construct a local image of the CDN by iterating through the XML file list and copying the CDN files, while creating the folder structure as needed.</span></span> 
   ...
   - <span data-ttu-id="400f6-327">Используйте URL-адрес + relativePath + имя файла для построения CDN расположения.</span><span class="sxs-lookup"><span data-stu-id="400f6-327">Use URL + relativePath + filename to construct the CDN location.</span></span>
...
<span data-ttu-id="400f6-348">**На следующем рисунке показана схема рабочего процесса обновления клиента Office 365.**</span><span class="sxs-lookup"><span data-stu-id="400f6-348">**The following figure shows a diagram of the Office 365 Client Update workflow.**</span></span>
...
-  <span data-ttu-id="400f6-356">*Архитектура*: определяет архитектуру процессора, связанные с этим обновлением.</span><span class="sxs-lookup"><span data-stu-id="400f6-356">*Arch*: Identifies the processor architecture associated with this update.</span></span> 
...
<span data-ttu-id="400f6-367">XML-файле содержится в подписанных CAB-ФАЙЛ и опубликовано CDN Office по следующему URL-АДРЕСУ: [http://officecdn.microsoft.com/pr/wsus/skus.cab](http://officecdn.microsoft.com/pr/wsus/skus.cab).</span><span class="sxs-lookup"><span data-stu-id="400f6-367">This XML file is contained within a signed CAB and published to the Office CDN at the following URL: [http://officecdn.microsoft.com/pr/wsus/skus.cab](http://officecdn.microsoft.com/pr/wsus/skus.cab).</span></span>
...
<span data-ttu-id="400f6-369">** \<ReleaseInfo\> ** корневого узла содержит атрибут PublishedDate, который определяет дату, которая была опубликована в этом файле.</span><span class="sxs-lookup"><span data-stu-id="400f6-369">**\<ReleaseInfo\>** root node contains the PublishedDate attribute which identifies the date which this file was published.</span></span> 
  
<span data-ttu-id="400f6-370">** \<SKU\> ** узел определяет отдельные SKU.</span><span class="sxs-lookup"><span data-stu-id="400f6-370">**\<SKU\>** node identifies an individual SKU.</span></span> 
  
- <span data-ttu-id="400f6-371">Атрибут *ProductID* определяет идентификатор, который передается в качестве атрибута ID в configuration.xml при использовании ODT.</span><span class="sxs-lookup"><span data-stu-id="400f6-371">The  *ProductID*  attribute identifies the ID that is passed as the ID attribute in the configuration.xml if using the ODT.</span></span> <span data-ttu-id="400f6-372">Например, `<Product ID="O365ProPlusRetail">`.</span><span class="sxs-lookup"><span data-stu-id="400f6-372">For example, `<Product ID="O365ProPlusRetail">`.</span></span> 
    
- <span data-ttu-id="400f6-373">*Значение по умолчанию* атрибута, если присвоено значение True, идентифицирующее рекомендуемая Конфигурация.</span><span class="sxs-lookup"><span data-stu-id="400f6-373">The  *Default*  attribute, if set to True, identifies the recommended SKU.</span></span> 
    
<span data-ttu-id="400f6-374">** \<Приложения\> ** узлов используются для определения отдельных приложений Office, поддерживаемых каждой SKU.</span><span class="sxs-lookup"><span data-stu-id="400f6-374">**\<App\>** nodes are used to define the individual Office apps that each SKU supports.</span></span> 
  
- <span data-ttu-id="400f6-375">Атрибут *Name* — это имя отображаемой приложения.</span><span class="sxs-lookup"><span data-stu-id="400f6-375">The  *Name*  attribute is the displayed application name.</span></span> 
    
- <span data-ttu-id="400f6-376">Атрибут *AppID* является в атрибуте ID, переданного файла configuration.xml для ** \<ExcludeApp\> ** узел при использовании ODT.</span><span class="sxs-lookup"><span data-stu-id="400f6-376">The  *AppID*  attribute is the ID attribute passed in the configuration.xml for the **\<ExcludeApp\>** node if using the ODT.</span></span> <span data-ttu-id="400f6-377">Например, `<ExcludeApp ID="Publisher" />`.</span><span class="sxs-lookup"><span data-stu-id="400f6-377">For example, `<ExcludeApp ID="Publisher" />`.</span></span> 
    
<span data-ttu-id="400f6-378">**RELEASEHISTORY. XML**</span><span class="sxs-lookup"><span data-stu-id="400f6-378">**RELEASEHISTORY.XML**</span></span>
  
<span data-ttu-id="400f6-379">Этот XML-файл содержится в подписанных CAB-ФАЙЛ и публикации в CDN Office в следующем расположении: [http://officecdn.microsoft.com/pr/wsus/releasehistory.cab](http://officecdn.microsoft.com/pr/wsus/releasehistory.cab).</span><span class="sxs-lookup"><span data-stu-id="400f6-379">This XML file is contained within a signed CAB and published to the Office CDN at the following location: [http://officecdn.microsoft.com/pr/wsus/releasehistory.cab](http://officecdn.microsoft.com/pr/wsus/releasehistory.cab).</span></span> 
  
<span data-ttu-id="400f6-380">Метаданные, опубликованной в XML-файл полезен при определении, какие каналы поддерживаются для обслуживания обновления из сети CDN Office, а также сведения о журнале построения для каждого из поддерживаемых каналов.</span><span class="sxs-lookup"><span data-stu-id="400f6-380">The metadata published in this XML file is useful for determining which channels are supported for servicing updates from the Office CDN along with information about the build history for each of the supported channels.</span></span>
...
<span data-ttu-id="400f6-381">** \<ReleaseHistory\> ** корневого узла содержит атрибут PublishedDate, который определяет дату, которая была опубликована в этом файле.</span><span class="sxs-lookup"><span data-stu-id="400f6-381">The **\<ReleaseHistory\>** root node contains the PublishedDate attribute which identifies the date which this file was published.</span></span> 
  
<span data-ttu-id="400f6-382">** \<UpdateChannel\> ** узел определяет поддерживаемые канала.</span><span class="sxs-lookup"><span data-stu-id="400f6-382">The **\<UpdateChannel\>** node defines a supported channel.</span></span> 
  
- <span data-ttu-id="400f6-383">Атрибут *Name* определяет идентификатор канала, который используется для передачи ODT в configuration.xml как атрибут канала.</span><span class="sxs-lookup"><span data-stu-id="400f6-383">The  *Name*  attribute defines the channel ID which is used to pass to the ODT in the configuration.xml as the Channel attribute.</span></span> 
    
  <span data-ttu-id="400f6-384">Пример: `<Add SourcePath="\\Server\Share" OfficeClientEdition="32" Channel="Current">`</span><span class="sxs-lookup"><span data-stu-id="400f6-384">Example: `<Add SourcePath="\\Server\Share" OfficeClientEdition="32" Channel="Current">`</span></span> 
    
  > [!NOTE] 
  > <span data-ttu-id="400f6-385">Этот атрибут является устаревшим и используется для обеспечения обратной совместимости.</span><span class="sxs-lookup"><span data-stu-id="400f6-385">This attribute has been deprecated and is used for backward compatibility only.</span></span> <span data-ttu-id="400f6-386">Использование атрибута ID вместо атрибута Name.</span><span class="sxs-lookup"><span data-stu-id="400f6-386">Use the ID attribute in place of the Name attribute.</span></span> 
    
- <span data-ttu-id="400f6-387">В атрибуте *ID* определяет идентификатор канала, который используется для передачи ODT в configuration.xml как атрибут канала.</span><span class="sxs-lookup"><span data-stu-id="400f6-387">The  *ID*  attribute defines the channel ID which is used to pass to the ODT in the configuration.xml as the Channel attribute.</span></span> 
    
  <span data-ttu-id="400f6-388">Пример: `<Add SourcePath="\\Server\Share" OfficeClientEdition="32" Channel="Deferred">`</span><span class="sxs-lookup"><span data-stu-id="400f6-388">Example: `<Add SourcePath="\\Server\Share" OfficeClientEdition="32" Channel="Deferred">`</span></span> 
    
- <span data-ttu-id="400f6-389">Атрибут **DisplayName** используется в качестве отображаемого имени.</span><span class="sxs-lookup"><span data-stu-id="400f6-389">The **DisplayName**  attribute is used as the display name.</span></span> 
    
<span data-ttu-id="400f6-390">** \<Обновление\> ** узел используется для определения каждого обновления, который был опубликован для конкретного канала.</span><span class="sxs-lookup"><span data-stu-id="400f6-390">The **\<Update\>** node is used to define each update that has been published for that particular channel.</span></span> 
  
- <span data-ttu-id="400f6-391">**Новейшие** атрибут, если значение True, определяет выпуска, который является последнего выпуска для этого канала.</span><span class="sxs-lookup"><span data-stu-id="400f6-391">The **Latest**  attribute, if set to True, defines the release that is the latest release for that channel.</span></span> 
    
- <span data-ttu-id="400f6-392">Атрибут **Version** определяет номер версии для этого конкретного обновления.</span><span class="sxs-lookup"><span data-stu-id="400f6-392">The **Version** attribute defines the version number for this particular update.</span></span> 
    
- <span data-ttu-id="400f6-393">Атрибут **LegacyVersion** определяет полный номер версии для этого конкретного обновления.</span><span class="sxs-lookup"><span data-stu-id="400f6-393">The **LegacyVersion** attribute defines the full version number for this particular update.</span></span> 
    
- <span data-ttu-id="400f6-394">Атрибут **построения** определяет номер сборки для данного конкретного обновления.</span><span class="sxs-lookup"><span data-stu-id="400f6-394">The **Build** attribute defines the build number for this particular update.</span></span> 
    
- <span data-ttu-id="400f6-395">Атрибут **PubTime** определяет дату и время публикации это обновление Office CDN.</span><span class="sxs-lookup"><span data-stu-id="400f6-395">The **PubTime** attribute defines the date and time at which this update was published to the Office CDN.</span></span> 
