...
|Поддержка определения DPI на уровне системы     |   Приложения определяют разрешение основного подключенного монитора при входе в Windows, но не могут реагировать на изменение разрешения. Дополнительные сведения см. в разделе [Настройка Windows для исправления размытых приложений](#Configure-Windows-to-fix-blurry-apps) этой статьи.      |     Приложение — растровое изображение, растягиваемое при перемещении на новый дисплей с другим разрешением.    |
...
Сообщение [WM_DPICHANGED](https://msdn.microsoft.com/en-us/library/windows/desktop/dn312083(v=vs.85).aspx) отправляется при изменении разрешения для окна.  В неуправляемом коде это сообщение обрабатывается с помощью [процедуры окна](https://msdn.microsoft.com/en-us/library/windows/desktop/ms633570(v=vs.85).aspx) для свойства HWND.  Пример кода обработчика изменения разрешения можно найти в статье [WM_DPICHANGED](https://msdn.microsoft.com/en-us/library/windows/desktop/dn312083(v=vs.85).aspx). 
...
- 
  [Создание классических приложений и приложений Win32 с поддержкой определения DPI.](https://msdn.microsoft.com/en-us/library/windows/desktop/dn469266(v=vs.85).aspx) Представляет общие сведения и руководство по созданию классических приложений Win32. Многие из методов, описанных в этой статье применяются для решений по расширению Office.
- 
  [API для смешанного использования режимов масштабирования и поддержки определения DPI.](https://msdn.microsoft.com/en-us/library/windows/desktop/mt744321(v=vs.85).aspx) Содержит список API, относящихся к разрешению.
...
- [Маркер DPI_AWARENESS_CONTEXT](https://docs.microsoft.com/ru-RU/windows/desktop/hidpi/dpi-awareness-context). Содержит сведения о значениях и определениях DPI_AWARENESS_CONTEXT для программирования.
- [Разработка классических приложений с высоким разрешением для Windows.](https://docs.microsoft.com/ru-RU/windows/desktop/hidpi/high-dpi-desktop-application-development-on-windows#testing-your-changes) Содержит сведения о тестировании в разделе "Тестирование изменений".
