...
Предоставляемый COM-объект используется в качестве сервера вне процесса. Указание КЛСКТКС_ЛОКАЛ_СЕРВЕР в составе компонента CoCreateInstance обеспечивает возможность совместимости между 64 64 и 32 – 32 процессами.
...
|E_FAIL  <br/> |Вызов завершился ошибкой.  <br/> |
|Е_ЛСК_ФИЛЕНОТФАУНД  <br/> |Данный ResourceID не находится в кэше.  <br/> |
|Е_ЛСК_НОТИНИТИАЛИЗЕД  <br/> |Инициализация не была успешно вызвана в прошлое.  <br/> |
|Е_ЛСК_ПЕНДИНГЧАНЖЕСИНКАЧЕ  <br/> |Файл в настоящее время синхронизируется или открывается, и его невозможно удалить.  <br/> |
...
|E_FAIL  <br/> |Вызов завершился ошибкой.  <br/> |
|E_INVALIDARG  <br/> |Один или несколько параметров являются недопустимыми.  <br/> |
|Е_ЛСК_НОТИНИТИАЛИЗЕД  <br/> |[Илсклокалсинкклиент:: инициализация](using-csisyncclient-to-control-the-office-document-cache-odc.md#ILSCLocalSyncClient_Initialize) не была успешно вызвана в прошлое.  <br/> |
|S_OK  <br/> |The call succeeded.  <br/> |
...
|E_FAIL  <br/> |Вызов завершился ошибкой.  <br/> |
|E_INVALIDARG  <br/> |Один или несколько параметров являются недопустимыми.  <br/> |
|Е_ЛСК_ФИЛЕНОТФАУНД  <br/> |Сведения о файле, указанные в _бстрресаурцеид_ , не существуют в кэше.  <br/> |
|Е_ЛСК_ЛОКАЛФИЛЕУНАВАИЛАБЛЕ  <br/> |Запрошен Лскстатусфлаг_локалфилеунчанжед или файл, указанный в _бстрресаурцеид_ , заблокирован или отсутствует.  <br/> |
|Е_ЛСК_НОТИНИТИАЛИЗЕД  <br/> |[Илсклокалсинкклиент:: инициализация](using-csisyncclient-to-control-the-office-document-cache-odc.md#ILSCLocalSyncClient_Initialize) не была успешно вызвана в прошлое.  <br/> |
|S_OK  <br/> |The call succeeded.  <br/> |
...
Инициализация должна быть первым вызываемым методом. В противном случае все остальные API будут возвращать Е_ЛСК_НОТИНИТИАЛИЗЕД. При вызове метода Initialize для уже инициализированного объекта возвращается значение S_OK и ничего не происходит. Если возвращается Е_ЛСК_КАЧЕМИСМАТЧ, вызывающий может вызвать [илсклокалсинкклиент:: ресеткаче](using-csisyncclient-to-control-the-office-document-cache-odc.md#ILSCLocalSyncClient_ResetCache) , чтобы удалить кэш, связанный с заданным супплиедид. Однако в этом случае другие API все равно будут возвращать Е_ЛСК_НОТИНИТИАЛИЗЕД. 
...
|E_FAIL  <br/> |Вызов завершился ошибкой.  <br/> |
|E_INVALIDARG  <br/> |Один или несколько параметров являются недопустимыми.  <br/> |
|Е_ЛСК_КАЧЕМИСМАТЧ  <br/> |Супплиедид уже имеет связанный с ним кэш, но имеет другой ProgId или Филесистемдиректорихинт, чем предоставленные.  <br/> |
|Е_ЛСК_ДИРЕКТОРИХИНТКОНФЛИКТ  <br/> |Филесистемдиректорихинт (или вложенная папка) уже существует в другом кэше.  <br/> |
|Е_ЛАК_ПРОГИДКОНФЛИКТ  <br/> |ProgID уже существует в другом кэше.  <br/> |
|S_OK  <br/> |The call succeeded.  <br/> |
...
Локалфилечанже используется, чтобы сообщить COM-объекту Ксисинкклиент попытаться отправить указанный файл. Метод подготовит файл для отправки, включая чтение текущего содержимого файла. Если отправка уже ожидается, Предыдущая отправка будет отменена и новое содержимое подготовлено к отправке. Если файл открыт для редактирования в приложении, этот метод возвратит значение S_OK, не подготавливая файл для отправки (при наличии изменений приложение должно выполнить это действие).
...
Строка, которая определяет файл на сервере. Это значение должно быть непустым, допустимым URL-АДРЕСом, но не более чем ИНТЕРНЕТ_МАКС_УРЛ_ЛЕНГС, https://support.microsoft.com/kb/208427как определено. 
...
|E_FAIL  <br/> |Вызов завершился ошибкой.  <br/> |
|E_INVALIDARG  <br/> |Один или несколько параметров являются недопустимыми.  <br/> |
|Е_ЛСК_КОНФЛИКТИНГФИЛЕ  <br/> |Значение ResourceID для файла, указанного в параметре _бстрфилесистемпас_ , отличается от указанного. При возвращении этой ошибки отправляется событие типа Лсцевенттипе_онфилепасконфликт. См. [илсклокалсинкклиент:: Changes ](using-csisyncclient-to-control-the-office-document-cache-odc.md#ILSCLocalSyncClient_GetChanges).  <br/> |
|Е_ЛСК_ФИЛЕНОТФАУНД  <br/> |Файл был удален средней операцией.  <br/> |
|Е_ЛСК_ФИЛЕНОТСУППОРТЕД  <br/> |Объект COM Ксисинкклиент не поддерживает заданное расширение файла. См. [илсклокалсинкклиент:: жетсуппортедфиликстенсионс ](using-csisyncclient-to-control-the-office-document-cache-odc.md#ILSCLocalSyncClient_GetSupportedFileExtensions).  <br/> |
|Е_ЛСК_ФИЛЕУПТОДАТЕ  <br/> |COM-объект не запланировать отправку, так как файл в кэше содержал последние изменения с диска.  <br/> |
|Е_ЛСК_ЛОКАЛФИЛЕУНАВАИЛАБЛЕ  <br/> |Файл, указанный в параметре _бстрфилесистемпас_ , отсутствует или заблокирован.  <br/> |
|Е_ЛСК_ЛОКАЛПАСНОТМАППЕД  <br/> |Данный Филесистемпас не находится в корневом каталоге, заданном параметром Филесистемдиректорихинт при вызове метода Initialize.  <br/> |
|Е_ЛСК_НОТИНИТИАЛИЗЕД  <br/> |[Илсклокалсинкклиент:: инициализация](using-csisyncclient-to-control-the-office-document-cache-odc.md#ILSCLocalSyncClient_Initialize) не была успешно вызвана в прошлое.  <br/> |
|Е_ЛСК_ПАСМИСМАТЧ  <br/> |Файл, указанный в параметре _бстрресаурцеид_ , отличается от указанного в параметре филесистемпас.  <br/> |
|Е_ЛСК_ПЕНДИНГЧАНЖЕСИНКАЧЕ  <br/> |Указанный файл уже содержит ожидающие изменения в другом кэше и еще не может быть связан с кэшем потребителя.  <br/> |
|Е_ЛСК_СЕРВЕРПАСИНДИФФЕРЕНТКАЧЕ  <br/> |Указанный путь находится в другом кэше.  <br/> |
|S_OK  <br/> |The call succeeded.  <br/> |
...
Строка, указывающая новый URL-адрес файла. Это значение должно быть непустым допустимым URL-АДРЕСом, но не превышать ИНТЕРНЕТ_МАКС_УРЛ_ЛЕНГС, как https://support.microsoft.com/kb/208427определено. 
...
|E_FAIL  <br/> |Вызов завершился ошибкой.  <br/> |
|E_INVALIDARG  <br/> |Один или несколько параметров являются недопустимыми.  <br/> |
|Е_ЛСК_КОНФЛИКТИНГФИЛЕ  <br/> |_Бстрневфилесистемпас_ или _бстрневвебпас_ уже существуют в другом файле в любом кэше. При возвращении этой ошибки отправляется событие типа Лсцевенттипе_онфилепасконфликт. См. [илсклокалсинкклиент:: Changes ](using-csisyncclient-to-control-the-office-document-cache-odc.md#ILSCLocalSyncClient_GetChanges).  <br/> |
|Е_ЛСК_ФИЛЕНОТФАУНД  <br/> |При выполнении этого метода сведения о файле были удалены из кэша.  <br/> |
|Е_ЛСК_ЛОКАЛПАСНОТМАППЕД  <br/> |Данный Филесистемпас не находится в корневом каталоге, заданном параметром Филесистемдиректорихинт при вызове метода Initialize.  <br/> |
|Е_ЛСК_НОТИНИТИАЛИЗЕД  <br/> |[Илсклокалсинкклиент:: инициализация](using-csisyncclient-to-control-the-office-document-cache-odc.md#ILSCLocalSyncClient_Initialize) не была успешно вызвана в прошлое.  <br/> |
|Е_ЛСК_ПЕНДИНГЧАНЖЕСИНКАЧЕ  <br/> |В настоящий момент указанный файл синхронизируется в приложении Office.  <br/> |
|S_OK  <br/> |The call succeeded.  <br/> |
...
Серверфилечанже указывает COM-объекту Ксисинкклиент пометить указанный файл для загрузки. Если файл открыт в приложении Office для редактирования, этот метод возвратит значение S_OK, не помечая файл для загрузки (при наличии изменений приложение должно выполнить это действие).
...
|Бстрвебпас  <br/> |Строка, которая определяет файл на сервере. Это значение должно быть непустым допустимым URL-АДРЕСом, но не превышать ИНТЕРНЕТ_МАКС_УРЛ_ЛЕНГС, как определено https://support.microsoft.com/kb/208427.  <br/> |
...
|Е_ЛСК_КОНФЛИКТИНГФИЛЕ  <br/> |Значение ResourceID для файла, указанного в параметре _бстрфилесистемпас_ , отличается от указанного.  <br/> |
|Е_ЛСК_ФИЛЕНОТСУППОРТЕД  <br/> |Объект COM Ксисинкклиент не поддерживает заданное расширение файла. Обратитесь к разделу Жетсуппортедфиликстенсионс.  <br/> |
|Е_ЛСК_ФИЛЕНОТФАУНД  <br/> |Файл был удален в средней операции.  <br/> |
|E_INVALIDARG  <br/> |Один или несколько параметров являются недопустимыми.  <br/> |
|Е_ЛСК_ЛОКАЛПАСНОТМАППЕД  <br/> |Данный Филесистемпас не находится в корневом каталоге, заданном параметром Филесистемдиректорихинт при вызове метода Initialize.  <br/> |
|Е_ЛСК_НОИНИТИАЛИЗЕД  <br/> |[Илсклокалсинкклиент:: инициализация](using-csisyncclient-to-control-the-office-document-cache-odc.md#ILSCLocalSyncClient_Initialize) не была успешно вызвана в прошлое.  <br/> |
|Е_ЛСК_ПАСМИСМАТЧ  <br/> |Файл, указанный в параметре _бстрресаурцеид_ , отличается от указанного в параметре филесистемпас.  <br/> |
|Е_ЛСК_ПЕНДИНГЧАНЖЕСИНКАЧЕ  <br/> |Указанный файл уже содержит ожидающие изменения в другом кэше и еще не может быть связан с кэшем потребителя.  <br/> |
|Е_ЛСК_СЕРВЕРПАСИНДИФФЕРЕНТКАЧЕ  <br/> |Указанный путь находится в другом кэше.  <br/> |
|S_OK  <br/> |The call succeeded.  <br/> |
...
Это значение заполняется только в том случае, если переЧисление [лсцевенттипе](using-csisyncclient-to-control-the-office-document-cache-odc.md#Enum_LSCEventType) для события равно лсцевенттипе_онлокалконфликтстатечанжед. 
...
Это значение заполняется только в том случае, если переЧисление [лсцевенттипе](using-csisyncclient-to-control-the-office-document-cache-odc.md#Enum_LSCEventType) для события равно Лсцевенттипе_онсерверчанжесдовнлоадед или лсцевенттипе_онлокалчанжесуплоадед. 
...
Это значение заполняется только в том случае, если переЧисление [лсцевенттипе](using-csisyncclient-to-control-the-office-document-cache-odc.md#Enum_LSCEventType) для события равно лсцевенттипе_онфилепасконфликт. 
...
|:-----|:-----|
|Лсцевентсинцеррортипе_усеринтервентионрекуиредунекспектед  <br/> |Ошибка синхронизации этого события, которая может требовать особой важности. По умолчанию пользователю может потребоваться поСледовать.  <br/> |
|Лсцевентсинцеррортипе_ноинтервентионрекуиред  <br/> |Ошибка синхронизации этого события не требует особой важности. Office будет автоматически обрабатывать его.  <br/> |
|Лсцевентсинцеррортипе_усеринтервентионрекуиред  <br/> |Для ошибки синхронизации этого события необходимо, чтобы пользователь решил эту проблему. Например, для ошибки слияния необходимо, чтобы пользователь открыл документ, а затем слить его.  <br/> |
|Лсцевентсинцеррортипе_ваитингонклиент  <br/> |Для ошибки синхронизации этого события необходимо, чтобы потребитель натребовался, но не обязательно должен быть особым вопросом.  <br/> |
|Лсцевентсинцеррортипе_клиентинтервентионрекуиред  <br/> |Для ошибки синхронизации этого события необходимо, чтобы потребитель был особым случаем.  <br/> |
|Лсцевентсинцеррортипе_макс  <br/> ||
...
|Перечисляем|Описание|
|:-----|:-----|
|Лсцевенттипе_ноне  <br/> ||
|Лсцевенттипе_онлокалчанжес  <br/> |Изменения, внесенные в локальный файл.  <br/> |
|Лсцевенттипе_онопенедбюсер  <br/> |Пользователь открыл файл.  <br/> |
|Лсцевенттипе_онсерверчанжесдовнлоадед  <br/> |Загрузка изменений файлов с сервера завершена.  <br/> |
|Лсцевенттипе_онлокалчанжесуплоадед  <br/> |Отгрузка изменений файлов на сервер завершена.  <br/> |
|Лсцевенттипе_онлокалконфликтстатечанжед  <br/> |Изменено состояние конфликта слияния файла.  <br/> |
|Лсцевенттипе_онфилеаддед  <br/> |Добавлен файл.  <br/> |
|Лсцевенттипе_онфиледелетед  <br/> |Удален файл.  <br/> |
|Лсцевенттипе_онсинценаблед  <br/> |Синхронизация для файлов пользователя включена.  <br/> |
|Лсцевенттипе_онсерверчанжесдовнлоадстартед  <br/> |Началась загрузка изменений файлов с сервера.  <br/> |
|Лсцевенттипе_онлокалчанжесуплоадстартед  <br/> |Начата отправка изменений файлов на сервер.  <br/> |
|Лсцевенттипе_онфилепасконфликт  <br/> |Это событие создается при вызове [илсклокалсинкклиент:: локалфилечанже ](using-csisyncclient-to-control-the-office-document-cache-odc.md#ILSCLocalSyncClient_LocalFileChange), [Илсклокалсинкклиент:: серверфилечанже ](using-csisyncclient-to-control-the-office-document-cache-odc.md#ILSCLocalSyncClient_ServerFileChange)или [Илсклокалсинкклиент:: RenameFile](using-csisyncclient-to-control-the-office-document-cache-odc.md#ILSCLocalSyncClient_RenameFile) вызывает веб-путь или конфликт локального пути с другим файлом в Кэш файлов Office.  <br/> |
|Лсцевенттипе_онфилефоркед  <br/> ||
|Лсцевенттипе_макс  <br/> ||
...
|Перечисляем|Описание|
|:-----|:-----|
|Лсцевенттипеоккурред_жетчанжес  <br/> |Произошла Илсцевент. Потребитель должен вызывать [илсклокалсинкклиент::](using-csisyncclient-to-control-the-office-document-cache-odc.md#ILSCLocalSyncClient_GetChanges) -Changes для получения данных.  <br/> |
|Лсцевенттипеоккурред_жетсуппортедфиликстенсионс  <br/> |Изменены поддерживаемые расширения файлов. Потребитель должен вызвать [илсклокалсинкклиент:: жетсуппортедфиликстенсионс](using-csisyncclient-to-control-the-office-document-cache-odc.md#ILSCLocalSyncClient_GetSupportedFileExtensions) , чтобы получить новый список поддерживаемых расширений.  <br/> |
...
|Перечисляем|Описание|
|:-----|:-----|
|Лскнетворксинкпермиссионтипе_хигхкост  <br/> |Значение true, если эвристика затрат для дорогостоящих сетей (например, 3G) переопределена.  <br/> |
|Лскнетворксинкпермиссионтипе_хигхповерусаже  <br/> |Имеет значение true, если переопределяется эвристика затрат для использования питания (например, батарея).  <br/> |
...
|Перечисляем|Описание|
|:-----|:-----|
|Лксстатусфлаг_ноне  <br/> ||
|Лскстатусфлаг_уплоадпендинг  <br/> |Значение true, если имеются ожидающие данные для отправки в файл сервера.  <br/> |
|Лскстатусфлаг_довнлоадпендинг  <br/> |Значение true, если ожидаются данные, которые требуется скачать из файла сервера.  <br/> |
|Лскстатусфлаг_локалфилеунчанжед  <br/> |Значение true, если в кэше данных в файле есть последняя копия данных на диске.  <br/> |
