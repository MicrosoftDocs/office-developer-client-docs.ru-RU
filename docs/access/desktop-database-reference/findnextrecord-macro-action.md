...
> [!TIP]
> Если в качестве аргумента " **только текущий поле** " в макрокоманде **НайтиЗапись** задано значение **"Да"**, может потребоваться использовать макрокоманду **КЭлементуУправления** для перемещения фокуса на элемент управления с искомыми данными перед использованием действия **финднекстрекорд** .
