...
В отличие от других коллекций ADO, использование метода **Refresh** в коллекции **Fields** не оказывает никакого действия. Чтобы получить изменения из базовой структуры базы данных, необходимо использовать метод reQuery **** или, если объект **Recordset** не поддерживает закладки, метод **MoveFirst** , который вызывает выполнение команды для Поставщик еще раз.
