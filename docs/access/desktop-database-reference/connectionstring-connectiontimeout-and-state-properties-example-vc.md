...
В этом примере демонстрируются различные способы использования свойства [ConnectionString](connectionstring-property-ado.md) для открытия [объекта Connection.](connection-object-ado.md) Оно также использует свойство [ConnectionTimeout,](connectiontimeout-property-ado.md) чтобы установить период времени ожидания подключения, и свойство [State](state-property-ado.md) для проверки состояния подключений. Для запуска этой процедуры требуется функция GetState.
