...
В этом примере используется [метод](supports-method-ado.md) Supports для отображения параметров, которые поддерживаются набором записей с разными типами курсоров. Для выполнения этой процедуры требуется процедура Дисплайсуппорт.
