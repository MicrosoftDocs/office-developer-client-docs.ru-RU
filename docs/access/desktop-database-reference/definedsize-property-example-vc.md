...
В этом примере показано [свойство DefinedSize](definedsize-property-adox.md) [столбца.](column-object-adox.md) Код будет переопределять размер столбца FirstName таблицы **Сотрудников** базы данных *Northwind.* Затем отображается изменение значений поля FirstName в [](recordset-object-ado.md) наборе записей на основе таблицы **Employees.** [](field-object-ado.md) Обратите внимание, что по умолчанию поле FirstName становится заполнено пробелами после пересмотра свойства **DefinedSize.**
