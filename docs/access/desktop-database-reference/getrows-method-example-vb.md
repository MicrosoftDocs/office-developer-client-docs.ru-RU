...
В этом примере используется метод [GetRows](getrows-method-ado.md) для получения указанного числа строк из объекта [Recordset](recordset-object-ado.md) и для заполнения массива итоговыми данными. Метод **GetRows** возвращает меньше строк, чем нужно в двух случаях: если достигнуто значение [EOF](bof-eof-properties-ado.md) или метод **GetRows** пытается получить запись, удаленную другим пользователем. Функция возвращает значение **False** только при возникновении второго случая. Функция GetRowsOK необходима для запуска этой процедуры.
