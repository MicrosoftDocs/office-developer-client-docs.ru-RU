...
Хотя обычное использование объекта **Command** не является типичным, в следующем коде показан базовый метод использования объекта **Command** для выполнения команды в отношении источника данных. В этом случае это команда, возвращаемая строками, поэтому она возвращает результаты выполнения команды в объект **Recordset.**
...
Команда, которая будет выполнена, указывается **свойством CommandText.**


> [!NOTE]
> В нескольких примерах в этом разделе называются утилита **GetNewConnection,** чтобы установить подключение к поставщику данных. Чтобы избежать избыточности, он перечислены только один раз:
