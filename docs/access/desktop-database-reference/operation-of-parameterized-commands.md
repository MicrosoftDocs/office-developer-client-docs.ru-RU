...
Родительская и дочерняя таблицы имеют имя столбца Common ID,\_идентификатор Cust *.* Дочерняя *команда* имеет заполнитель "_км_", к которому относится предложение Relate (то есть "... ПАРАМЕТР 0 ").
...
Чтобы получить требуемый результат, можно использовать параметризованную команду. Пример:
