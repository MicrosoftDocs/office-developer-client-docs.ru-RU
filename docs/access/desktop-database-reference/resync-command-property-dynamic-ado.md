...
Задает пользовательскую строку команды, которая выдает [](resync-method-ado.md) метод Resync для обновления данных в таблице, имя которой указано в динамическом свойстве [таблицы Unique](unique-table-unique-schema-unique-catalog-properties-dynamic-ado.md) .
