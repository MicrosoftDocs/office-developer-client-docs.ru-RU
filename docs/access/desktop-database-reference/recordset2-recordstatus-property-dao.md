...
Когда пользователь изменяет запись, **recordStatus** для этой записи автоматически изменяется на **dbRecordModified**. Аналогично, если запись добавляется или удаляется, **RecordStatus** отражает соответствующую константу. При использовании метода обновления **[](recordset2-update-method-dao.md)** в пакетном режиме DAO будет отправлять соответствующую операцию на удаленный сервер для каждой записи на основе свойства **RecordStatus** записи.

## <a name="example"></a>Пример

В этом примере используются свойства **RecordStatus** и **DefaultCursorDriver,** чтобы показать, как отслеживаются изменения локального объекта **Recordset** во время пакетного обновления. Для запуска этой процедуры требуется функция RecordStatusOutput.
