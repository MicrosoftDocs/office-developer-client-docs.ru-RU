...
Перемещает положение текущей записью в объекте [Recordset](recordset-object-ado.md).
...
|*Start* |Необязательный атрибут. **Строковое** значение или **Variant** , вычисление которого дает закладку. Вы также можете использовать значение [букмаркенум](bookmarkenum.md) .|
...
Если для локального кэширования записей из поставщика используется свойство [CacheSize](cachesize-property-ado.md) , передача аргумента *нумрекордс* , который перемещает текущую запись вне текущей группы кэшированных записей, заставляет ADO получить новую группу записей. начиная с целевой записи. Свойство **CacheSize** определяет размер вновь извлеченной группы, а конечную запись — первую извлеченную запись.
