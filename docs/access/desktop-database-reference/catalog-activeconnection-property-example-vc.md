...
Установка для [свойства ActiveConnection](activeconnection-property-adox.md) допустимого открытого подключения "открывает" каталог. Из открытого каталога можно получить доступ к объектам схемы, которые содержатся в этом каталоге.
