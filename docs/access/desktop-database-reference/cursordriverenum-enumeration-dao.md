...
<td><p>–1</p></td>
<td><p>(По умолчанию) Использует курсоры на стороне сервера, если сервер поддерживает их; в противном случае используется библиотека курсоров ODBC.</p></td>
</tr>
<tr class="odd">
<td><p>dbUseNoCursor</p></td>
<td><p>4 </p></td>
<td><p>Открывает все курсоры (то есть <strong>объекты Recordset)</strong> как тип только для чтения, только для чтения, с размером строки 1. Также известен как &quot; запросы без курсоров.&quot;</p></td>
</tr>
<tr class="even">
...
<td><p>2</p></td>
<td><p>Всегда использует курсоры на стороне сервера. Для большинства крупных операций этот параметр обеспечивает лучшую производительность, но может привести к большему сетевому трафику.</p></td>
</tr>
</tbody>
</table>
