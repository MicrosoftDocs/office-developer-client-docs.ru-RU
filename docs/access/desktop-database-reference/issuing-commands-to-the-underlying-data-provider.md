...
Любая команда, которая не начинается с SHAPE, передается поставщику данных. Это эквивалентно выдаче команды фигуры в виде "SHAPE {provider command}". Эти команды не *должны* создавать набор **записей.** Например, "SHAPE {DROP TABLE MyTable} — это совершенно допустимая команда фигуры, если поставщик данных поддерживает DROP TABLE.

Эта возможность позволяет обычным командам поставщика и командам фигур совместно использовать одно подключение и транзакцию.
