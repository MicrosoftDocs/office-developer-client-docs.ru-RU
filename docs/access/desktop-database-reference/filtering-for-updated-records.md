...
В этом примере показано, как расширить предыдущий пример **UpdateBatch** , выполнив фильтрацию **набора записей** непосредственно перед вызовом **UpdateBatch**, отображая пользователя, какие записи будут изменены, и позволяя ему отменить обновление (с помощью метода **CancelBatch** ).
