...
Первая функция в этом примере использует свойства [BOF](bof-eof-properties-ado.md) и [EOF](bof-eof-properties-ado.md) для отображения сообщения, если пользователь пытается перейти мимо первой или последней записи [recordset](recordset-object-ado.md). Оно использует свойство [Bookmark,](bookmark-property-ado.md) чтобы позволить пользователю пометить запись в **наборе Recordset** и вернуться к ней позже.

Вторая функция использует свойство Bookmark  для того, чтобы разместить закладки любой другой записи из **наборов записей** в массив, а затем фильтровать набор записей с помощью массива.
