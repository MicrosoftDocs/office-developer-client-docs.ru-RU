...
Используйте **свойство PageCount,** чтобы определить, сколько страниц данных находится в **объекте Recordset.** *Страницы —* это группы записей, размер которых равен **параметру свойства PageSize.** Даже если последняя страница является неполной, так как записей меньше, чем значения **PageSize,** она считается дополнительной страницей в значении **PageCount.** Если объект **Recordset** не поддерживает это свойство, **pageCount** будет 1, чтобы указать, что **PageCount** неопределен.
...
Используйте свойство **AbsolutePage,** чтобы определить номер страницы, на котором расположена текущая запись. Опять же, поставщик должен поддерживать соответствующие функции для этого свойства, которые будут доступны.

**AbsolutePage** 1-based и равно 1, когда текущая запись является первой записью в **Наборе записей.** Установите это свойство, чтобы перейти к первой записи определенной страницы. Получение общего числа страниц из свойства **PageCount.**
