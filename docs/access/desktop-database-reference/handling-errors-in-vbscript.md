...
Методы, используемые в Visual Basic и VBScript, практически не отличаются. Основное различие состоит в том, что VBScript не поддерживает концепцию обработки ошибок, продолжая выполнение в метку. Другими словами, нельзя использовать On Error GoTo в VBScript. Вместо этого используйте в VBScript. Вместо этого используйте on Error Resume Next, а затем проверьте **Err.Number** и свойство **Count** коллекции **Errors,** как показано в следующем примере:
