...
Можно также использовать псевдонимы таблиц во вложенном запросе для обращения к таблицам, перечисленных в предложении [FROM](https://docs.microsoft.com/office/vba/access/Concepts/Structured-Query-Language/from-clause-microsoft-access-sql) за пределами подчиненного запроса. Следующий пример возвращает имена сотрудников, зарплата которых равно или больше средней заработной платы всех сотрудников, занимающих одну должность. В таблице сотрудников задается псевдоним «Т1».
