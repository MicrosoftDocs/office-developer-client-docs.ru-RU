...
<td><p>Начальный набор строк, извлеченный текущим запросом, был сохранен в <strong>наборе записей</strong> <strong>объекта данных</strong> и доступен для использования. Остальные строки все еще выбираются.</p></td>
</tr>
<tr class="odd">
<td><p><strong>адкреадистатекомплете</strong></p></td>
<td><p>Все строки, извлеченные текущим запросом, были сохранены в <strong>наборе записей</strong> <strong>объекта данных</strong> и доступны для использования. Это состояние также будет существовать, если операция прервана из-за ошибки или объект <strong>Recordset</strong> не инициализирован.</p></td>
</tr>
</tbody>
</table>
...
Если во время асинхронной операции возникает ошибка, свойство **ReadyState** изменяется на **Адкреадистатекомплете**, свойство [State](state-property-ado.md) изменяется с **адстатиксекутинг** на **адстатеклосед**, а свойство [value](value-property-ado.md) объекта **Recordset** *не*остается.
