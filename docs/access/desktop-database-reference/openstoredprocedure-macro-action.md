...
> [!TIP]
> - Хранимую процедуру можно перетаскивать из области переходов в строку действие в макросе. Это автоматически создает **ОткрытьСохраненнуюПроцедуру** действие, которое будет открыта хранимую процедуру в режиме таблицы.
> - Если вы не хотите вывод системных сообщений, которые появляются при запуске хранимую процедуру (это хранимые процедуры и показывают количество записей затрагиваются), можно использовать действие **УстановитьСообщения** для отмены вывода на отображение сообщения.
