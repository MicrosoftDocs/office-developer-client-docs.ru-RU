...
Параметр для **DefaultUser** имеет тип данных String. Это может быть длина от 1 до 20 символов в рабочих областях Microsoft Access, которая может содержать буквы, цифры, пробелы и символы, кроме: "(кавычек),/(косая черта), \\ (обратная косая черта) \[ \] , (скобки) : (двоеточие), | (pipe), \< (знак "меньше"), \> (знак "больше"), + (знак плюс), = (знак равенства),; (точка с запятой), (запятая),? (вопросительный знак) \* , (звездочка), начальные пробелы и управляющие символы (ASCII 00 — ASCII 31).
