...
Эти три примерах показано, как [Сохранить](save-method-ado.md) и [Открыть](open-method-ado-recordset.md) методы можно использовать вместе.
...
Во-первых доступа и сохраните таблицу ***авторов*** .
...
На этом этапе вы попали в месте назначения. Доступ к таблице ***авторов*** будет как локального, отключенные **набора записей**. Не забудьте необходимо иметь поставщика **MSPersist** на компьютере, который используется для доступа к сохраненный файл ответ:\\Pubs.xml.
...
И, наконец можно вернуться на домашнюю страницу. Теперь обновление базы данных с помощью изменения.
