...
---
# <a name="save-and-open-methods-example-vb"></a>Пример использования методов Save и Open (VB)


**Область применения**: Access 2013, Office 2013

В этих трех примерах [](open-method-ado-recordset.md) показано, как можно совместно использовать методы [Сохранить](save-method-ado.md) и Открыть.

Предположим, что вы собираетесь в командировку и хотите взять таблицу из базы данных. Перед входом вы можете получить доступ к данным в виде [наборов записей](recordset-object-ado.md) и сохранить их в транспортируемых формах. По прибытии в пункт назначения вы можете получить доступ к **Набору записей** в качестве локального отключенного **наборов записей.** Вы внося изменения в **Набор записей,** а затем сохраните его снова. Наконец, когда вы возвращаетесь домой, вы снова подключаетесь к базе данных и обновляете ее с помощью изменений, внесенных на дороге.
...
На данный момент вы прибыли в пункт назначения. Вы будете получать доступ к таблице ***Авторов*** в качестве локального отключенного **набор записей.** Не забывайте, что поставщик **MSPersist** должен быть на компьютере, который используется для доступа к сохраненным файлам, а именно: \\Pubs.xml.
...
Наконец, вы вернетесь домой. Теперь обновите базу данных с помощью изменений.
