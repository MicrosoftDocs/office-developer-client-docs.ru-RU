...
В этих трех примерах показано, как можно совместно использовать методы [Save](save-method-ado.md) и [Open](open-method-ado-recordset.md) .
...
На этом шаге вы попали в место назначения. Вы получите доступ к таблице ***authors*** в качестве локального, отключенного **набора записей**. Не забывайте, что у вас должен быть поставщик **мсперсист** на используемом компьютере, чтобы получить доступ к сохраненному файлу:\\Pubs. XML.
...
Наконец, вы вернете домой. Теперь обновите базу данных с учетом изменений.
