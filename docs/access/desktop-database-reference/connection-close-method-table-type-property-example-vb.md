...
Установка для [свойства ActiveConnection](activeconnection-property-adox.md) свойства **Nothing** должно "закрыть" каталог. Связанные коллекции будут пустыми. Все объекты, созданные из объектов схемы в каталоге, будут потерянными. Все свойства этих объектов, которые были кэшировали, по-прежнему будут доступны, но попытка прочитать свойства, которые требуют вызова поставщика, не удастся.
...
Закрытие объекта [Connection,](connection-object-ado.md) который использовался для "открытия" каталога, должно иметь тот же эффект, что и установка свойству **ActiveConnection** свойства **Nothing.**
