...
Указывает уровень изоляции для объекта [Connection.](connection-object-ado.md)
...
Свойство **IsolationLevel** имеет свойство read/write.

**Использование удаленной службы данных** При этом свойству **IsolationLevel** можно установить только **adXactUnspecified.**

Так как пользователи работают с отключенными объектами **Recordset** в клиентском кэше, могут возникнуть проблемы с несколькими объектами. Например, когда два разных пользователя пытаются обновить ту же запись, удаленная служба данных просто разрешает пользователю, который сначала обновляет запись, «win». Второй запрос на обновление пользователя не будет работать с ошибкой.
