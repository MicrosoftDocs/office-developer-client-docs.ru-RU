...
Задает или возвращает значение, которое указывает, является ли строка нулевой **[](field-value-property-dao.md)** длины ("") допустимым параметром для свойства Value объекта **Field2** с типом данных Text или Memo (только в рабочей области Microsoft Access).
...
В этом примере **свойство AllowZeroLength** позволяет пользователю установить значение **Field2** в пустой строке. В этой ситуации пользователь может различать запись, в которой данные не известны, и запись, в которой данные не применяются.
