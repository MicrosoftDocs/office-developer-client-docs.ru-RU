...
В этом примере используется метод [NextRecordset](nextrecordset-method-ado.md) для просмотра данных в наборе записей, в котором используется составной оператор команды, состоящего из трех отдельных операторов **SELECT** .
