...
The Visual C++ section of [ADO Event Instantiation by Language](https://docs.microsoft.com/office/client-developer/access/desktop-database-reference/ado-event-instantiation-by-language-ado) gives a general description of how to instantiate the ADO event model. Ниже приводится конкретный пример создания примера модели событий в среде, созданной **\# директивой импорта.**

В общем описании **adoint.h** используется в качестве ссылки на сигнатуры методов. Однако некоторые сведения в общем описании немного изменяются в результате использования **\# директивы импорта:**

- Директива **\# импорта** разрешит типы данных и модификаторы сигнатуры **typedef** и method в их фундаментальные формы.

- Все чисто виртуальные методы, которые необходимо перезаписать, имеют префикс **"raw". \_**

В некоторых кодах просто отражается стиль кодирования.

- Указатель на **IUnknown,** используемый методом **Advise,** получается явным образом при вызове **QueryInterface.**

- Вам не нужно явно кодировать деструктор в определениях класса.

- Вам может потребоваться код более надежных реализаций QueryInterface, AddRef и Release.

- Директива **\_ \_ uuidof()** широко используется для получения ИД интерфейса.

Наконец, в примере содержится рабочий код.

- Пример написан как консольное приложение.

- Вам следует вставить собственный код под комментарием "// Do some work".

- Все обработчики событий по умолчанию не делают ничего и отменяют дополнительные уведомления. Необходимо вставить соответствующий код для приложения и при необходимости разрешить уведомления.
