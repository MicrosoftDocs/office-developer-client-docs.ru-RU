...
Раздел Visual C++, в разделе [ADO Event Instantiation by Language,](https://docs.microsoft.com/office/client-developer/access/desktop-database-reference/ado-event-instantiation-by-language-ado) дает общее описание того, как мгновенно сменить модель событий ADO. Ниже приводится конкретный пример мгновенного создания модели событий в среде, созданной директивой **\# импорта.**

Общее описание использует **adoint.h** в качестве ссылки для подписей метода. Однако некоторые сведения в общем описании немного изменяются в результате использования директивы **\# импорта:**

- Директива **\# импорта** устраняет типы и модификаторы подписей **typedef's** и метод для основных форм.

- Чистые виртуальные методы, которые должны быть перезаписаны, префиксом являются **\_ "необработанные".**

Некоторые коды просто отражают стиль кодирования.

- Указатель на **IUnknown,** используемый методом **Advise,** получен явно с помощью вызова **в QueryInterface**.

- Вам не нужно явно кодировать деструктор в определениях класса.

- Возможно, вам нужно кодировать более надежные реализации в QueryInterface, AddRef и Release.

- Директива **\_ \_ uuidof()** широко используется для получения ID интерфейса.

Наконец, в примере содержится рабочий код.

- Пример написан как консольное приложение.

- Вы должны вставить собственный код под комментарием "// Сделайте некоторые работы".

- Все обработчики событий по умолчанию ничего не делают и отменяют дополнительные уведомления. Необходимо вставить соответствующий код для приложения и при необходимости разрешить уведомления.
