...
Свойство **Server** можно задать во время создания в элементе [ActiveX RDS. Объектные теги объекта DataObject или](datacontrol-object-rds.md) во время выполнения в коде скрипта.
...
|*порта* |Необязательное. Порт, используемый для подключения к серверу IIS. Номер порта задается в Internet Explorer (в меню **Сервис** выберите пункт **Свойства браузера**, а затем перейдите на вкладку **Подключение** ) или в IIS.|
|*DataControl* |Объектная переменная, представляющая **RDS. Объект управления** DataObject.|

## <a name="remarks"></a>Примечания

Сервер — это расположение, в котором **RDS. **Обрабатывается запрос на получение доступа к управлению запросами (то есть запрос или обновление). По умолчанию все запросы обрабатываются объектом [рдссервер.](datafactory-object-rdsserver.md) DataObject, [мсдфмап. Компонент обработчика](datafactory-customization.md) и [мсдфмап. INI](understanding-the-customization-file.md) -файл на указанном сервере. 
