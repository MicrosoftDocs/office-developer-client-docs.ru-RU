...
---
# <a name="stayinsync-property-example-vj"></a>Пример использования свойства StayInSync (VJ++)
...
Во внешнем цикле отображаются имя и фамилия, состояние и идентификация каждого автора. Appended **Recordset** для каждой строки получается из коллекции [Fields](fields-collection-ado.md) и автоматически передается **rstTitleAuthor** **свойством StayInSync** при каждом переходе родительского объекта **Recordset** к новой строке. Внутренний цикл отображает четыре поля из каждой строки в наборе записей.
