...
В следующем загодре, **icrsint.h,** подробно интерфейс, который позволяет клиентам извлекать поля из **recordset** в переменные, определенные в классе, производном от **CADORecordBinding**. Необходимо указать макрос привязки ADO для каждого поля, к чему вы собираетесь получить доступ.
