...
В следующем заговорке, **icrsint.h,** подробно сообщается об интерфейсе, который позволяет клиентам извлекать поля из **recordset** в переменные, определенные в классе, полученном из **CADORecordBinding.** Необходимо указать макрос привязки ADO для каждого поля, к чему вы собираетесь получить доступ.
