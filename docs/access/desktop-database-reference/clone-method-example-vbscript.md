...
В этом примере используется метод [Clone](clone-method-ado.md), чтобы создать копии объекта [Recordset](recordset-object-ado.md), после чего пользователь может разместить указатели записей для каждой копии независимо друг от друга.

Используйте следующий пример на странице Active Server Page (ASP). В этом примере используется база данных Northwind, распространенная с помощью Microsoft Access. Вырезать и ввести следующий код для Блокнот или другого текстового редактора и сохранить его как **CloneVBS.asp**. Результат можно просмотреть в любом клиентом браузере.

Чтобы осуществить пример, измените строку RsCustomerList.Source = "Клиенты" на RsCustomerList.Source = "Продукты" для подсчета большей таблицы.
