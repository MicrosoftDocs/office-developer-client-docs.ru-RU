...
Используйте следующий пример на странице Active Server (ASP). В этом примере используется база данных Northwind, распределенная с помощью Microsoft Access. Вырезать и ввести следующий код в Блокнот или другой текстовый редактор и сохранить его как **CloneVBS.asp**. Результат можно просмотреть в любом браузере клиента.

Чтобы осуществить пример, измените строку RsCustomerList.Source = "Customers" на RsCustomerList.Source = "Products" для подсчета таблицы большего размера.
