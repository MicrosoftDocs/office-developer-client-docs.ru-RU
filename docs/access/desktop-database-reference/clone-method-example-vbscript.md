...
Используйте приведенный ниже пример на активной серверной странице (ASP). В этом примере используется база данных Northwind, распределенная с помощью Microsoft Access. Скопируйте и вставьте следующий код в Блокнот или другой текстовый редактор и сохраните его как **клоневбс. ASP**. Результаты можно просмотреть в любом клиентском браузере.

Чтобы выполнить этот пример, измените строку Рскустомерлист. Source = "Customers" на Рскустомерлист. Source = "Products" для подсчета более крупной таблицы.
