...
В этом примере метод [OpenSchema используется](openschema-method-ado.md) для отображения имени и типа каждой таблицы в базе ***данных Pubs.***
...
В этом примере в аргументе Criteria метода OpenSchema указывается ограничение запроса \_ **ТИПА**  ТАБЛИЦЫ. В результате возвращаются только сведения о схеме для представлений, указанных в базе ***данных Pubs.*** Затем в примере отображаются имена и типы каждой таблицы.
