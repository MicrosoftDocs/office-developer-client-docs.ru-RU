...
В этом примере метод [OpenSchema отображает](openschema-method-ado.md) имя и тип каждой таблицы в ***базе данных Pubs.***
...
В этом примере указывается ограничение запроса ТИПА TABLE \_ в **аргументе Критерии метода OpenSchema.**  В результате возвращаются только сведения о схеме представлений, указанные в базе данных ***Pubs.*** В этом примере отображаются имя (s) и тип (ы) каждой таблицы(ы).
