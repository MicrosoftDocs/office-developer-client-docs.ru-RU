...
В этом примере демонстрируется метод [CompareBookmarks.](comparebookmarks-method-ado.md) Относительное значение закладок редко требуется, если определенная закладка не является чем-то особенным.

В качестве цели поиска назначим  случайный ряд [наборов](recordset-object-ado.md) записей, полученных из таблицы Авторов. Затем отобразить положение каждой строки относительно этой цели.
