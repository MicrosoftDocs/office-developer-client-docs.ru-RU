...
Параметры свойств **CollatingOrder** и **Attributes** объекта **Field2** в коллекции **Полей** объекта **Index** вместе определяют последовательность и направление порядка сортировки в индексе. Однако вы не можете установить порядок коллажи для отдельного индекса— его можно установить только для всей таблицы.
