...
Настраиваемые бизнес-объекты должны гарантировать, что клиентская сторона может соедидиствию имени программы (ProgId) с идентификатором (CLSID), который можно использовать через DCOM. По этой причине ProgID объекта DCOM должен быть в клиентском реестре и со картой с ИД класса бизнес-объекта на стороне сервера. Для других поддерживаемых протоколов (HTTP, HTTPS и в процессе) это необязательно.

Например, если вы предоставляете серверный бизнес-объект MyBObj с определенным ИД класса, например "{00112233-4455-6677-8899-00AABBCCDDEE}", убедитесь, что в клиентский реестр добавлены следующие записи:
