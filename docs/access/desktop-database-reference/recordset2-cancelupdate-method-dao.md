...
Проверьте свойство **[EditMode](recordset2-editmode-property-dao.md)** объекта **Recordset,** чтобы определить, есть ли ожидающих операций, которые можно отменить.

> [!NOTE]
> Использование метода **CancelUpdate** имеет тот же эффект, что и перемещение в другую запись без использования метода **[Update,](recordset2-update-method-dao.md)** за исключением того, что текущая запись не меняется, а различные свойства, такие как **[BOF](recordset2-bof-property-dao.md)** и **[EOF,](recordset2-eof-property-dao.md)** не обновляются.
