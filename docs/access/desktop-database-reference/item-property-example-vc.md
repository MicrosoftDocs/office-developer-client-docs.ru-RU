...
В этом примере показано, как [свойство Item](item-property-ado.md) имеет доступ к членам коллекции. В примере открывается таблица ***Авторов*** базы данных ***Pubs*** с помощью параметризированной команды.

Параметр в команде, выданной в базе данных, имеет [](parameters-collection-ado.md) доступ к коллекции Параметры объекта [Command](command-object-ado.md) по индексу и имени. Затем к полям возвращаемого [набору записей](recordset-object-ado.md) можно получить доступ из коллекции [Полей](fields-collection-ado.md) этого объекта по индексу и имени.
