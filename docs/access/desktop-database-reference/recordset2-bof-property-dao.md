...
В рабочем пространстве Microsoft Access, если добавить запись в пустой набор записей, **boF** станет **false,** но **EOF** останется **True,** указав, что текущая позиция находится в конце наборов записей.

Любой **метод Delete,** даже если он удаляет единственную оставшиеся записи из наборов записей, не изменит параметр **свойства BOF** или **EOF.**
