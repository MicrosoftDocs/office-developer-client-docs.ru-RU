...
В этом примере используется метод [AddNew](addnew-method-ado.md) , чтобы создать новую запись с указанным именем.

Используйте следующий пример в активную страницу сервера (ASP). Используйте **Поиск** для найдите файл Adovbs.inc и помещения их в каталог, который планируется использовать. Скопируйте и вставьте следующий код в блокноте или другом текстовом редакторе и сохраните файл с именем **AddNewVBS.asp**. Результат можно просмотреть в браузере любого клиента.

Чтобы использовать в примере, добавьте новую запись в формате HTML. Щелкните **Добавить**. В разделе [Удаление пример метода](delete-method-example-vbscript.md) удалите ненужные записи.
