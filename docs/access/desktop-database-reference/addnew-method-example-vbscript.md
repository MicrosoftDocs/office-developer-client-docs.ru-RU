...
Используйте приведенный ниже пример на активной серверной странице (ASP). Используйте **Find** , чтобы найти файл адовбс. Inc и разместить его в каталоге, который планируется использовать. Скопируйте и вставьте следующий код в Блокнот или другой текстовый редактор и сохраните его как **аддневвбс. ASP**. Результаты можно просмотреть в любом клиентском браузере.

Чтобы применить пример, добавьте новую запись в HTML-форму. Щелкните **Добавить**. Чтобы удалить ненужные записи, просмотрите [пример метода Delete](delete-method-example-vbscript.md) .
