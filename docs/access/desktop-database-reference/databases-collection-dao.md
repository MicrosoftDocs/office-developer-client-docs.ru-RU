...
Когда вы открываете существующий объект **Database** или создаете новый объект из **рабочей** области, он автоматически включается в коллекцию **databases.** При закрытии объекта **Database** с помощью метода **[Close](connection-close-method-dao.md)** он удаляется из коллекции **databases,** но не удаляется с диска. Перед закрытием объекта **Database** необходимо закрыть все открытые объекты **Recordset.**

В рабочей области Microsoft Access параметр свойства **Name** базы данных является строкой, которая указывает путь к файлу базы данных.

Чтобы сослаться на **объект Database** в коллекции по порядковому номеру или по его свойству **Name,** используйте любую из следующих синтаксис форм:
