...
Используйте метод **clone** для создания нескольких дубликатов объектов **Recordset** . Каждый набор записей может иметь собственную текущую запись. При **** использовании клона сам по себе данные не изменяются в объектах или в базовых структурах. При использовании метода **clone** можно предоставить общий доступ к закладкам между двумя или более объектами **Recordset2** , так как их закладки являются взаимозаменяемыми.
...
Использование метода **[Close](connection-close-method-dao.md)** для исходного или повторяющегося объекта не влияет на другой объект. Например, использование **Close** для исходного объекта Recordset не приводит к закрытию клона.

> [!NOTE]
> - Закрытие набора записей клонирования в ожидающей транзакции приводит к неявной операции **отката** .
> - При клонировании объекта **Recordset** табличного типа в рабочей области Microsoft Access параметр свойства **[index](recordset2-index-property-dao.md)** не клонируется в новой копии набора записей. Значение свойства **index** необходимо скопировать вручную.
