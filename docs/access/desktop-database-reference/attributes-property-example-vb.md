...
В этом примере показано [свойство Атрибуты](attributes-property-adox.md) [столбца.](column-object-adox.md) Настройка **adColNullable** позволяет пользователю задать значение поля [Recordset](recordset-object-ado.md)[пустой](field-object-ado.md) строке. В этой ситуации пользователь может различать запись, в которой данные не известны, и запись, в которой данные не применяются.
