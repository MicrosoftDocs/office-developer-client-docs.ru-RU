...
|*value* |Необязательное. **Логическое** значение **true** (по умолчанию), если требуется фильтровать по текущему набору строк "filtered". **Значение false** означает, что выполняется фильтрация по исходному набору строк с удалением всех предыдущих параметров фильтра.|
