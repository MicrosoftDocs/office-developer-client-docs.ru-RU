...
Задает или возвращает значение по умолчанию для **[объекта Field.](field-object-dao.md)** Для объекта **Field,** еще не присоединенного к коллекции **[Fields,](fields-collection-dao.md)** это свойство доступно только для чтения и записи (только для рабочей области Microsoft Access).
...
Значение параметра или возвращаемого значения — это **строка** типа данных, которая может содержать не более 255 символов. Это может быть текст или выражение. Если параметр свойства является выражением, он не может содержать пользовательские функции, общую базу данных Microsoft Access SQL функций, а также ссылки на запросы, формы или другие объекты **Field.**


> [!NOTE]
> Вы также можете установить для свойства **DefaultValue** объекта **Field** объекта [TableDef](tabledef-object-dao.md) специальное значение под названием "GenUniqueID( )". Это приводит к присвоению этому полю случайного числа при каждом добавлении или добавлении новой записи, предоставляя каждой записи уникальный идентификатор. Свойство Type поля [должно](field-type-property-dao.md) иметь **длину.**
...
При создании новой записи параметр **свойства DefaultValue** автоматически в качестве значения поля в качестве значения. Значение поля можно изменить, установив его свойство **[Value.](field-value-property-dao.md)**
