...
В этом примере свойство [Mode](mode-property-ado.md) используется для открытия эксклюзивного подключения, а [свойство IsolationLevel](isolationlevel-property-ado.md) — для открытия транзакции, которая проводится в изоляции от других транзакций.
