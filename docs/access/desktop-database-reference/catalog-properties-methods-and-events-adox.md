---
title: Свойства, методы и события объекта Catalog (ADOX)
TOCTitle: Catalog properties, methods, and events (ADOX)
ms:assetid: 82d7dfc2-6fc8-709c-96e0-d8cddd1d5432
ms:mtpsurl: https://msdn.microsoft.com/library/JJ249567(v=office.15)
ms:contentKeyID: 48545989
ms.date: 09/18/2015
mtps_version: v=office.15
ms.openlocfilehash: 1162b844ab31faa7fa529e87acf092464e3930a2
ms.sourcegitcommit: d7248f803002b31cf7fc561b03530199a9b0a8fd
ms.translationtype: MT
ms.contentlocale: ru-RU
ms.lasthandoff: 11/02/2018
ms.locfileid: "25930240"
---
# <a name="catalog-properties-methods-and-events-adox"></a>Свойства, методы и события объекта Catalog (ADOX)

**Применимо к**: Access 2013, Office 2013 

## <a name="propertiescollections"></a>Свойства и коллекции

- [Свойство ActiveConnection](activeconnection-property-adox.md)

- [Семейства сайтов групп](groups-collection-adox.md)

- [Процедуры семейства сайтов](procedures-collection-adox.md)

- [Коллекция таблиц](tables-collection-adox.md)

- [Пользователи семейства сайтов](users-collection-adox.md)

- [Коллекции представлений](views-collection-adox.md)

## <a name="methods"></a>Методы

- [Метод Create](create-method-adox.md)

- [Метод GetObjectOwner](getobjectowner-method-adox.md)

- [Метод SetObjectOwner](https://msdn.microsoft.com/library/jj249006\(v=office.15\))

## <a name="events"></a>Мероприятия

Нет.

