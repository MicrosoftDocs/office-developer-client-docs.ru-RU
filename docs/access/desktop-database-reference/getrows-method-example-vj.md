...
---
# <a name="getrows-method-example-vj"></a><span data-ttu-id="86c19-102">Пример использования метода GetRows (VJ++)</span><span class="sxs-lookup"><span data-stu-id="86c19-102">GetRows method example (VJ++)</span></span>


<span data-ttu-id="86c19-103">**Область применения**: Access 2013, Office 2013</span><span class="sxs-lookup"><span data-stu-id="86c19-103">**Applies to**: Access 2013, Office 2013</span></span>

<span data-ttu-id="86c19-104">В этом примере используется метод [GetRows](getrows-method-ado.md) для получения указанного числа строк из объекта [Recordset](recordset-object-ado.md) и для заполнения массива итоговыми данными.</span><span class="sxs-lookup"><span data-stu-id="86c19-104">This example uses the [GetRows](getrows-method-ado.md) method to retrieve a specified number of rows from a [Recordset](recordset-object-ado.md) and to fill an array with the resulting data.</span></span> <span data-ttu-id="86c19-105">Метод **GetRows** возвращает меньше необходимого количества строк в двух случаях: если [EOF](bof-eof-properties-ado.md) был достигнут, или если **GetRows** пытался получить запись, удаленную другим пользователем.</span><span class="sxs-lookup"><span data-stu-id="86c19-105">The **GetRows** method will return less than the desired number of rows in two cases: either if [EOF](bof-eof-properties-ado.md) has been reached, or if **GetRows** tried to retrieve a record that was deleted by another user.</span></span> <span data-ttu-id="86c19-106">Функция возвращает значение **False** только при возникновении второго случая.</span><span class="sxs-lookup"><span data-stu-id="86c19-106">The function returns **False** only if the second case occurs.</span></span> <span data-ttu-id="86c19-107">Функция GetRowsOK необходима для запуска этой процедуры.</span><span class="sxs-lookup"><span data-stu-id="86c19-107">The GetRowsOK function is required for this procedure to run.</span></span>
