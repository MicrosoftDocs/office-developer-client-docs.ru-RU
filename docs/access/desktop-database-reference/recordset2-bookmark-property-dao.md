...
Для объекта **Recordset,** полностью основанного на таблицах баз данных Microsoft Access, значение свойства **Bookmarkable** является True, и с этим набором записей можно использовать свойство **Bookmark.** Другие продукты базы данных, однако, могут не поддерживать закладки. Например, нельзя использовать закладки в любом объекте **Recordset2** на основе связанной таблицы Paradox, которая не имеет основного ключа.
...
В этом примере свойства **Bookmark** и **Bookmarkable** используются для того, чтобы позволить пользователю пометить запись в наборе записей и вернуться к ней позже.
