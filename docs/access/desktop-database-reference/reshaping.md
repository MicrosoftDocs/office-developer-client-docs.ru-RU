...
Вторая функция заключается в включении доступа, не разбитого на разделы, **** в существующие дочерние объекты `"SHAPE <recordset reshape name>"`Recordset с помощью синтаксиса.

> [!NOTE]
> Невозможно добавить столбцы к существующему **набору записей**, изменить форму параметризованного **набора записей** или объектов **Recordset** в любом промежуточном предложении COMPUTE или выполнить операции агрегированИя для любого потомка объекта **Recordset** из ** **Перерисовка объекта Recordset. Объект **Recordset** , для которого выполняется изменение формы, и команда Новая фигура должна использовать один и тот же объект[подключения](connection-object-ado.md) * *.
