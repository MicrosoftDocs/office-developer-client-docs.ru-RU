...
Ядро СУБД Microsoft Access использует индексы при присоединении таблиц и создании объектов **[Recordset](recordset-object-dao.md)** . Индексы определяют порядок, в котором объекты **Recordset** табличного типа возвращают записи, но они не определяют порядок, в котором ядро СУБД Microsoft Access хранит записи в базовой таблице, или порядок, в котором любой другой тип объекта **Recordset** возвращает записи.
