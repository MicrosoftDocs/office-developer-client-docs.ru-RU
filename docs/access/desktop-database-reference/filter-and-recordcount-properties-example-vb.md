...
В этом примере открывается **набор записей** в таблице Publishers базы ***данных Pubs.*** Затем свойство [Filter](filter-property-ado.md) используется для ограничения числа видимых записей для этих издателей в определенной стране или регионе. Свойство **RecordCount** используется для демонстрации различий между отфильтрованными и нефильтрованными записями.
...
> [!NOTE]
> Если вы знаете данные, которые вы хотите выбрать, обычно эффективнее открыть **набор записей** с помощью SQL. В этом примере показано, как создать только один **набор записей** и получить записи из определенной страны или региона.
