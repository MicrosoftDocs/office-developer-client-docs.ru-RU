...
В этом примере показано, как открыть объект **Recordset** в таблице издателей базы данных ***pubs*** . Затем с помощью свойства [Filter](filter-property-ado.md) можно ограничить количество видимых записей для этих издателей в определенной стране или регионе. Свойство **RecordCount** используется для отображения разницы между отфильтрованными и нефильтрованными наборами записей.
...
> [!NOTE]
> Если вы знаете данные, которые вы хотите выбрать, обычно более эффективно открыть объект **Recordset** с помощью оператора SQL. В этом примере показано, как создать только один объект **Recordset** и получить записи из определенной страны или региона.
