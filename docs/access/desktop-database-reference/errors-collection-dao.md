...
> [!NOTE]
> Если вы используете ключевое слово **New** для создания объекта, который вызывает ошибку до или во время помещения в коллекцию **Errors** , эта коллекция не содержит сведения об ошибке, так как новый объект не связан с Объект **DBEngine** . Однако сведения об ошибке доступны в объекте VBA **Err** .
