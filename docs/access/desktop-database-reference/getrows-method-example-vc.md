...
В этом примере используется метод [GetRows](getrows-method-ado.md) для получения указанного числа строк из объекта [Recordset](recordset-object-ado.md) и для заполнения массива итоговыми данными. Метод **GetRows** возвращает меньше необходимого количества строк в двух случаях: при достигнутом [EOF](bof-eof-properties-ado.md) или если **GetRows** попытался получить запись, удаленную другим пользователем. Функция возвращает значение **False** только при возникновении второго случая. Функция GetRowsOK необходима для запуска этой процедуры.
