---
title: Пространства данных (RDS) (ADO)
TOCTitle: DataSpace (RDS)
ms:assetid: eb9d33ab-e71d-42e0-bbfc-d7de80656dae
ms:mtpsurl: https://msdn.microsoft.com/library/JJ250196(v=office.15)
ms:contentKeyID: 48548494
ms.date: 09/18/2015
mtps_version: v=office.15
ms.openlocfilehash: ead8e19bb48ded0847d0c9f675242a607ee50c50
ms.sourcegitcommit: 48bfe5ab15b11105f4f52937b886c92bdc26525a
ms.translationtype: MT
ms.contentlocale: ru-RU
ms.lasthandoff: 11/02/2018
ms.locfileid: "25910678"
---
# <a name="dataspace-rds-ado"></a>Пространства данных (RDS) (ADO)

**Применимо к**: Access 2013, Office 2013

- [Объект DataSpace (RDS)](dataspace-object-rds.md)
- [Пространства данных свойства, методы и события (RDS)](dataspace-properties-methods-and-events-rds.md)

