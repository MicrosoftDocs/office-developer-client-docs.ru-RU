...
<td><p>Создает новый объект <strong><a href="relation-object-dao.md">Relation</a></strong> (только рабочие пространства Microsoft Access). .</p></td>
</tr>
<tr class="odd">
...
<td><p>Создает новую реплику из другой реплики базы данных (только в рабочей области Microsoft Access).</p></td>
</tr>
<tr class="even">
<td><p><strong><a href="database-newpassword-method-dao.md">NewPassword</a></strong></p></td>
<td><p>Изменяет пароль существующей базы данных базы данных Microsoft Access (только в рабочей области Microsoft Access).</p></td>
</tr>
<tr class="odd">
...
<td><p>Синхронизирует любые изменения частичной реплики с полной репликой, очищает все записи в частичной реплике, а затем повторно населяет частичную реплику на основе текущих фильтров реплики. (только базы данных баз данных microsoft Access.).</p></td>
</tr>
<tr class="odd">
...
<p>Возвращает объект <strong><a href="connection-object-dao.md">Connection,</a></strong> соответствующий базе данных (только в рабочей области ODBCDirect).</p></td>
</tr>
<tr class="even">
<td><p><strong><a href="database-containers-property-dao.md">Контейнеры</a></strong></p></td>
<td><p>Возвращает коллекцию <strong>Контейнеры,</strong> которая представляет все объекты <strong>контейнера</strong> в засвиде-ной базе данных. Только для чтения.</p></td>
</tr>
<tr class="odd">
<td><p><strong><a href="database-designmasterid-property-dao.md">DesignMasterID</a></strong></p></td>
<td><p>Задает или возвращает значение 16-byte, которое уникально определяет мастер дизайна в наборе реплик (только в рабочей области Microsoft Access).</p></td>
</tr>
<tr class="even">
...
<td><p>Задает или возвращает значение, которое указывает количество секунд, которые необходимо ждать до возникновения ошибки при выполнении запроса в источнике данных ODBC.</p></td>
</tr>
<tr class="even">
<td><p><strong><a href="database-recordsaffected-property-dao.md">RecordsAffected</a></strong></p></td>
<td><p>Возвращает количество записей, затронутых самым недавно вызываемой методом <strong><a href="connection-execute-method-dao.md">Выполнения.</a></strong></p></td>
</tr>
<tr class="odd">
<td><p><strong><a href="database-recordsets-property-dao.md">Recordsets</a></strong></p></td>
<td><p>Возвращает <strong>коллекцию Recordsets,</strong> которая содержит все открытые записи в указанной базе данных. Только для чтения.</p></td>
</tr>
<tr class="even">
...
<td><p>Возвращает значение 16-byte, которое уникально идентифицирует реплику базы данных (только в рабочей области Microsoft Access).</p></td>
</tr>
<tr class="even">
<td><p><strong><a href="database-tabledefs-property-dao.md">TableDefs</a></strong></p></td>
<td><p>Возвращает коллекцию <strong>TableDefs,</strong> которая содержит все объекты <strong>TableDef,</strong> хранимые в указанной базе данных. Только для чтения.</p></td>
</tr>
<tr class="odd">
...
<td><p>В рабочей области Microsoft Access возвращается весяк двигателя базы данных Microsoft Jet или Microsoft Access, создав базу данных. Только для чтения, <strong>String</strong>.</p></td>
</tr>
</tbody>
</table>
