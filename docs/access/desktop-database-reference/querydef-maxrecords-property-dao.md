...
Задает или возвращает максимальное число записей, возвращаемого из запроса к источнику данных ODBC.
...
Значение по умолчанию — 0, указывающее на отсутствие ограничений на количество возвращаемой записи.

После возврата в приложение в наборе записей количества **[строк,](recordset-object-dao.md)** указанных **maxRecords,** обработчик запросов перестанет возвращать дополнительные записи, даже если больше записей будут иметь право на включение в набор **записей.** Это свойство полезно в ситуациях, когда ограниченные клиентские ресурсы запрещают управление большим количеством записей.
...
В этом примере свойство **MaxRecords** используется для ограничения на количество записей, возвращаемой запросом к источнику данных ODBC.
