...
|Параметр|Описание|
|:--------|:----------|
|*Destination* |Необязательное. **Переменная типа Variant** , представляющая полный путь к файлу, в котором будет сохранен **набор записей** , или ссылку на объект **Stream** .|
|*персистформат* |Необязательное. Значение [персистформатенум](persistformatenum.md) , задающее формат, в котором будет сохранен **набор записей** (XML или адтг). Значение по умолчанию — **адперсистадтг**.|
