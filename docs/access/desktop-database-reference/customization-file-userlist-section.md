...
Раздел **списка пользователей** относится к разделу **connect** с тем же параметром *идентификатора* раздела.

В этом разделе может содержаться запись доступа *пользователя,* которая указывает права   доступа для указанного пользователя и переопределяет запись доступа по умолчанию в разделе подключения, **совпадающих.**

## <a name="syntax"></a>Синтаксис

Запись доступа пользователя имеет форму:
...
<td><p>Имя <em>пользователя,</em> применяющий это подключение. Допустимые имена пользователей устанавливаются в диалоговом оке "Диспетчер <strong>служб</strong> IIS".</p></td>
</tr>
<tr class="even">
