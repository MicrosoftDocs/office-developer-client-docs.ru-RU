...
В этом  примере открывается набор записей в таблице Companies базы данных Northwind, а затем свойство [Filter](filter-property-ado.md) используется для ограничения записей, видимых для тех, где поле CompanyName начинается с буквы D. Вырезать и в paste следующий код в Блокноте или другом текстовом редакторе и сохранить его как **FilterJS.asp**.
