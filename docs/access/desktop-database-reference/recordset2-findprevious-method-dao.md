...
> [!NOTE]
> - Для лучшей производительности критерии *** должны быть в форме *"значение* поля", где поле является индексным полем в базовой базовой таблице, или "поле LIKE префикс", где поле является индексным полем в базовой базовой таблице, а префикс — строкой поиска префикса  =  (например, "ART*").     
