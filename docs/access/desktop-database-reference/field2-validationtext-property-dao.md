...
Задает или возвращает значение, задающее текст сообщения, которое отображается в приложении, если значение объекта **field2** не соответствует правилу проверки, заданному свойством **ValidationRule** (только для рабочих областей Microsoft Access). Для чтения и записи, **String**.
...
Для объекта **field2** использование свойства **ValidationText** зависит от объекта, содержащего коллекцию **[Fields](fields-collection-dao.md)** , к которой добавляется объект **field2** , как показано в следующей таблице.
