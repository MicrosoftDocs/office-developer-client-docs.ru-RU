...
В этом примере метод [GetRows](getrows-method-ado.md) позволяет извлекать все строки таблицы *Custiomers* из [recordset](recordset-object-ado.md) и заполнять массив полученными данными. Метод **GetRows** возвращает меньше строк, чем нужно в двух случаях: если достигнуто значение [EOF](bof-eof-properties-ado.md) или метод **GetRows** пытается получить запись, удаленную другим пользователем. Функция возвращает значение **False** только при возникновении второго случая. Вырезать и вклеить следующий код для Блокнот или другого редактора текста и сохранить его как **GetRowsJS.asp**.
