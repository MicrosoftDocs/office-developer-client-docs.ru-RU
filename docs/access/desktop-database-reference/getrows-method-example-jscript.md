...
В этом примере метод [GetRows](getrows-method-ado.md) используется для получения всех строк *таблицы Custiomers* из [recordset](recordset-object-ado.md) и заполнения массива полученными данными. Метод **GetRows** возвращает меньше строк, чем нужно в двух случаях: если достигнуто значение [EOF](bof-eof-properties-ado.md) или метод **GetRows** пытается получить запись, удаленную другим пользователем. Функция возвращает значение **False** только при возникновении второго случая. Включите следующий код в Блокнот или другой текстовый редактор и сохраните его как **GetRowsJS.asp.**
