...
В этом примере демонстрируются методы [GetObjectOwner](getobjectowner-method-adox.md) и [SetObjectOwner.](https://docs.microsoft.com/office/vba/access/concepts/miscellaneous/setobjectowner-method-adox) В этом коде предполагается, что существует группа Accounting (см. пример методов [Groups и Users Append, ChangePassword (VC++),](groups-and-users-append-changepassword-methods-example-vc.md) чтобы узнать, как добавить эту группу в систему). Для владельца таблицы "Категории" установлено "Бухгалтерия".
