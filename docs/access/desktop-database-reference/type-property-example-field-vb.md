...
В этом примере показано свойство [Type](type-property-ado.md) , которое отображает имя константы, которое соответствует значению свойства [Type](type-property-ado.md) всех объектов [field](field-object-ado.md) в таблице ***Employees*** . Для выполнения этой процедуры требуется функция FieldType.
