...
В этом примере используется свойство [State](state-property-ado.md) для отображения сообщения, в то время как асинхронные подключения открываются и выполняются асинхронные команды.
