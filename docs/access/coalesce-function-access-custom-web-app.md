...
---
# <a name="coalesce-function-access-custom-web-app"></a>Функция Coalesce (Доступ к настраиваемой веб-приложению)
...
**Coalesce** (*Значение*, [*Значение*], ...,[*Значение*]) 
...
Если все аргументы являются NULL, **Coalesce** возвращает NULL. 
  
## <a name="example"></a>Пример

В качестве правила проверки для таблицы используется следующее выражение. Выражение гарантирует, что записи будут сделаны в полях Имя, Фамилия, Электронная почта, Мобильные Телефон, Work Телефон, Home Телефон и Company перед записью. Если любое из перечисленных полей остается пустым, функция **Coalesce** возвращает Null, что нарушает правило проверки. 
