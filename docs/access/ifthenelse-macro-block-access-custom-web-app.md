...
---
# <a name="ifthenelse-macro-block-access-custom-web-app"></a>Если... Затем... Else Macro Block (Access custom web app)
...
Для **if** и ** Else If **требуются следующие аргументы. 
...
В следующем примере кода действия макроса в первом блоке выполняются, если значение [Status] превышает 0. Если значение [Status] не превышает 0, выражение, которое  следует за другим, оценивается. Действия макроса в **блоке Else If** выполняются, если значение [Status] равно 0. Наконец, если ложны выражения как в первом, так и во втором блоке, выполняются макрокоманды блока **Иначе**. 
...
Вы можете вкладывать блоки **Если** друг в друга. Один блок **Если** можно вложить в другой, если в случае истинности первого выражения нужно оценить второе. В следующем примере кода внутренний блок **If** выполняется только в том случае, если значение [Status] превышает  *0*  и более 100. 
