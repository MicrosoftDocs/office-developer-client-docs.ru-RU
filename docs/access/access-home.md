---
title: Access
description: Здесь вы найдете инструкции, примеры кода, документацию по API и пакетам SDK, справочные материалы по VBA, учебные материалы и технические статьи, посвященные разработке решений и настройке Access.
ms.date: 10/16/2018
ms.audience: Developer
ms.assetid: 9858d097-d1d1-48df-8344-e7b46afbe0d1
localization_priority: Priority
ms.openlocfilehash: c32fec61ead1433581f528ebefff7b0f4b59df62
ms.sourcegitcommit: 8fe462c32b91c87911942c188f3445e85a54137c
ms.translationtype: HT
ms.contentlocale: ru-RU
ms.lasthandoff: 04/23/2019
ms.locfileid: "32279590"
---
# <a name="access"></a>Access

Здесь вы найдете инструкции, примеры кода, документацию по API и пакетам SDK, справочные материалы по VBA, учебные материалы и технические статьи, посвященные разработке решений и настройке Access.
  
### <a name="in-this-section"></a>В этом разделе
  
- [Справочник по VBA для Access](https://docs.microsoft.com/office/vba/api/overview/access)
- [Справочник по пользовательским веб-приложениям для Access](https://docs.microsoft.com/office/client-developer/access/access-custom-web-app-reference)  
- [Справочник по базам данных Access на компьютере](https://docs.microsoft.com/office/client-developer/access/desktop-database-reference/)
  
### <a name="office-add-ins"></a>Надстройки Office
  
- [Начало работы](https://docs.microsoft.com/office/dev/add-ins/)  
- [Обзор платформы](https://docs.microsoft.com/office/dev/add-ins/overview/office-add-ins)
  
### <a name="other-resources"></a>Другие ресурсы

- [Access 2010](https://docs.microsoft.com/previous-versions/office/developer/office-2010/ff604965(v=office.14)) 
- [Справочник по библиотеке объектов VBA для Office](https://docs.microsoft.com/office/vba/api/overview/library-reference)  
- [Справочник по VBA для Office](https://docs.microsoft.com/office/vba/api/overview/language-reference) 
- [Open XML SDK](https://docs.microsoft.com/office/open-xml/open-xml-sdk) 
- [Надстройки VSTO для Office](https://docs.microsoft.com/visualstudio/vsto/create-vsto-add-ins-for-office-by-using-visual-studio?view=vs-2017)
  
### <a name="support"></a>Поддержка
  
- [Центр разработчиков Office](https://developer.microsoft.com/office) 
- [Поддержка Office](https://support.office.com/) 
- [Форум, посвященный Access, в Центре разработчиков Office](https://social.msdn.microsoft.com/Forums/office/home?forum=accessdev)
  

