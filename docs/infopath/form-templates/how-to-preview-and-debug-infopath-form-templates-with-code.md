...
---
# <a name="preview-and-debug-infopath-form-templates-with-code"></a>Просмотр и отладка шаблонов форм InfoPath с кодом
...
> [!NOTE]
> Отладка шаблонов форм после развертывания во время выполнения из Visual Studio 2012 невозможна. Этот компонент включает шаблонов форм, совместимые только с InfoPath, а также тех, совместимые с InfoPath, так и веб-браузер, с помощью службы InfoPath Forms Services. Тем не менее можно запись значений поля из кода во время выполнения при отладке бизнес-логике шаблона формы. Сведения о том, как сделать, просмотра [Журнала значений в поле для отладки](how-to-log-values-to-a-field-for-debugging.md). 
