...
COM-объекты, доступ к которым предоставляется через сборки взаимодействия Microsoft.Office.Interop.InfoPath.dll, Microsoft.Office.Interop.InfoPath.SemiTrust.dll и Microsoft.Office.Interop.InfoPath.Xml.dll, установленные приложением Microsoft InfoPath, не поддерживают вызовы в нескольких потоках. К ним относятся интерфейсы для объектов MSXML (MSXML), которые включаются в пространство имен **Microsoft.Office.Interop.InfoPath.SemiTrust** (большинство из которых имеют имена с префиксом IXMLDOM), а также все интерфейсы, которые данной областью имен [Microsoft.Office.Interop.InfoPath.Xml,](https://msdn.microsoft.com/library/microsoft.office.interop.infopath.xml)  ни один из которых не потокобезопасн. 
