...
## <a name="example-1-sorting-data-in-an-order-form"></a>Пример 1. Сортировка данных в форме порядка
...
- Обратитесь к администратору фермы, чтобы убедиться, что служба песочницы Microsoft SharePoint Foundation запущена на сервере. Дополнительные сведения [см. в публикации форм с кодом.](publishing-forms-with-code.md)
    
- Выбранный для шаблона форм язык программирования — это **C#** или **Visual Basic** без указания какого-либо номера более ранней версии после названия. Версии языков программирования и объектных моделей, совместимые с InfoPath 2007 и InfoPath 2003, не поддерживаются для решений в песочнице. Дополнительные сведения о том, как задать язык программирования, см. в [Visual Studio.](how-to-develop-with-visual-studio.md)
...
5. Опубликуйте форму с помощью следующих действий:
...
1. Создайте простой настраиваемый список на сервере SharePoint Server 2010 и назначйте ему имя MyList. В следующем примере используется **поле со списком,** привязанное к полю **Title** этого списка. 
    
2. Создайте **новую** пустую форму в конструкторе InfoPath, вставьте в форму поле со **combo** и переименуйте поле, привязанное к полю со combo, на myCombo.
    
3. Создайте подключение к данным для списка, который будет использоваться для заполнения окна со списком, вы можете использовать следующие действия:
...
4. Заполнять значения в поле со списком из списка можно с помощью следующих действий:
    ...
    6. На **вкладке "Формы браузера"** выберите "Всегда в параметрах", а затем нажмите кнопку **"ОК",** чтобы закрыть диалоговое окно свойств.   
    
5. Убедитесь, что поле со combo все еще выбрано, а затем нажмите кнопку **"Событие** изменено" на вкладке **"Разработчик"** ленты. 
    
    Если форма еще не была сохранена, система предложит сохранить ее. Затем откроется окно редактора кода с курсором в  `myCombo_Changed` обработчике событий. 
    
6. Добавьте ссылку на сборку Microsoft.SharePoint.dll, как описано выше. Дополнительные сведения о ссылке на сборку Microsoft.SharePoint см. в подразделе ["Использование членов объектной модели SharePoint".](how-to-use-sharepoint-object-model-members.md)
    
7. В paste the following code into the  `myCombo_Changed` event handler. 
...
8. Предыдущий пример кода зависит от  `GetDomValue` функции-помощника. В paste the following code for the  `GetDomValue` helper function below the  `myCombo_Changed` event handler function. 
...
9. Опубликуйте форму с помощью следующих действий:
    ...
    5. Введите имя и описание библиотеки форм и нажмите кнопку **"Далее".**
        
    6. Нажмите кнопку **Опубликовать**.
        
    7. После успешной публикации формы откройте форму из библиотеки форм и добавьте новое значение в поле со списоком, чтобы проверить код. При выходе из поля myCombo новое значение будет записано в список SharePoint. 
