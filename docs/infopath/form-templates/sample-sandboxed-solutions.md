...
- Обратитесь к администратору фермы, чтобы убедиться, что на сервере SharePoint Microsoft SharePoint служба кода в песочнице. Дополнительные сведения см. в [публикации Форм с кодом.](publishing-forms-with-code.md)
    
- Выбранный для шаблона форм язык программирования — это **C#** или **Visual Basic** без указания какого-либо номера более ранней версии после названия. Версии языков программирования и объектных моделей, совместимые с InfoPath 2007 и InfoPath 2003, не поддерживаются для решений в песочнице. Дополнительные сведения о том, как указать язык программирования, см. в [Visual Studio.](how-to-develop-with-visual-studio.md)
...
## <a name="example-2-managing-vendors-in-a-sharepoint-list"></a>Пример 2. Управление поставщиками в SharePoint списке
...
1. Создайте простой настраиваемый список на сервере SharePoint Server 2010 и назови его MyList. В следующем примере используется **поле Combo Box,** связанное с **полем Title** этого списка. 
    
2. Создайте новую **пустую** форму в конструкторе InfoPath, вставьте в форму управление полем **combo** и переименуйте поле, связанное с полем комбо, в myCombo.
    
3. Создайте подключение к списку данных, которое будет использоваться для заполнения комбо-окна с помощью следующих действий:
...
4. Заполнять значения в поле комбо из списка с помощью следующих действий:
    ...
    6. На **вкладке Формы браузера** выберите **Всегда** под настройками **Postback,** а затем нажмите **кнопку ОК,** чтобы закрыть диалоговое окно свойств. 
    
5. Убедитесь, что поле комбо по-прежнему выбрано, а затем нажмите **кнопку Измененное** событие на вкладке **Разработчик** ленты. 
    
    Если форма еще не была сохранена, система предложит сохранить ее. Затем окно редактора кода открывается с помощью курсора в  `myCombo_Changed` обработчике событий. 
    
6. Добавьте ссылку на сборку Microsoft.SharePoint.dll, как описано выше. Дополнительные сведения о ссылке на Microsoft. SharePoint сборки см. [в SharePoint члены объектной модели.](how-to-use-sharepoint-object-model-members.md)
    
7. Вклеить следующий код в  `myCombo_Changed` обработник событий. 
...
8. Предыдущий пример кода зависит от  `GetDomValue` функции помощника. Вклеить следующий код для функции  `GetDomValue` помощника ниже функции  `myCombo_Changed` обработщика событий. 
...
9. Опубликуйте форму с помощью следующих действий:
    ...
    7. После успешной публикации формы откройте форму из библиотеки форм и добавьте новое значение в поле комбо для проверки кода. При выходе из поля myCombo новое значение будет записано в SharePoint. 
