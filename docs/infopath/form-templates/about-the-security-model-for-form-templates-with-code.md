...
---
# <a name="about-the-security-model-for-form-templates-with-code"></a>О модели безопасности для шаблонов форм с кодом
...
|Полное доверие  <br/> |Позволяет запускать форму с полным доверием на компьютере, где будет использоваться форма. Этот уровень безопасности можно использовать только при работе с формой, которая имеет цифровую подпись, которая соответствует надежному корневому издателю на компьютере, или путем создания программы установки, которая устанавливает форму и задает атрибут **requireFullTrust** элемента **xDocumentClass** как "yes" в файле определения формы (XSF). С помощью этого параметра форма может получать доступ к вызовам объектной модели, которые требуют уровня безопасности объектной модели 3, например свойства и методы, которые имеют доступ к файловой системе, и можно отключить определенные запросы безопасности, которые отображаются при запуске на более строгом уровне безопасности.  <br/> |
   
По умолчанию форма InfoPath настроена на автоматический выбор уровня безопасности "Ограниченный доступ" или "Домен" в зависимости от функций, используемых в шаблоне формы, а также места и места развертывания шаблона формы. Например, шаблон формы, развернутый как вложение электронной почты, автоматически настраивается на уровень безопасности "Ограниченный". Параметр безопасности всегда является как можно более строгим, начиная с Restricted, чтобы обеспечить более высокий уровень защиты для вас и ваших данных. Если шаблон формы, содержащий управляемый код, настроен для автоматического выбора уровня безопасности, шаблону формы всегда требуется по крайней мере уровень доступа к домену, прежде чем код сможет запускаться. Этот параметр можно переопрепредить вручную во время разработки, чтобы выбрать уровень безопасности, который больше подходит для формы, с помощью следующей процедуры. 
...
> [!NOTE] 
> - Если для шаблона формы с управляемым кодом выбрать уровень безопасности **Ограниченный**, то код формы не будет загружаться и запускаться независимо от того, какие элементы объектной модели используются в этом коде. Этот уровень безопасности в основном предназначен для форм InfoPath, которые развертываются с помощью электронной почты.     
> - Если выбрать уровень безопасности **Полное доверие**, то необходимо поставить цифровую подпись или установить и зарегистрировать формы. Дополнительные сведения см. в под [названием "Развертывание шаблонов форм InfoPath с кодом".](how-to-deploy-infopath-form-templates-with-code.md)
...
|:-----|:-----|:-----|:-----|:-----|:-----|:-----|
|Ограниченный доступ  <br/> ||X  <br/> |Нет ActiveX для всех  <br/> |Fail  <br/> |Ферма загружается, но управляемый код не запускается.  <br/> |0  <br/> |
...
Сведения о создании и настройке группы кода "Шаблоны форм InfoPath" см. в подстройке "Настройка параметров безопасности для шаблонов форм [с кодом".](how-to-configure-security-settings-for-form-templates-with-code.md)
