...
9. На следующей странице мастера нажмите  кнопку "Формула" рядом с полем "Имя вложения", а затем повторите вышеперечисленные действия, чтобы создать конкату ("Отчет о продажах - ", точка") и нажмите кнопку  "Далее". 
    
10. На последней странице мастера введите "Отправить по электронной почте" в поле "Введите имя для этого подключения к данным" и нажмите кнопку **"Готово".** 
...
3. Добавьте следующие две функции под обработом событий [Отправка:](https://msdn.microsoft.com/library/Microsoft.Office.InfoPath.FormEvents.Submit.aspx) 
...
4. Добавьте следующий запрос **if** в [функцию обработера](https://msdn.microsoft.com/library/Microsoft.Office.InfoPath.FormEvents.Submit.aspx) событий Submit. 
