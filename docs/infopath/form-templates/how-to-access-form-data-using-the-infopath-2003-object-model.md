...
Доступ **к интерфейсу XDocumentsCollection** можно получить с помощью свойства [XDocuments](https://msdn.microsoft.com/library/Microsoft.Office.Interop.InfoPath.SemiTrust._Application2.XDocuments.aspx) [интерфейса](https://msdn.microsoft.com/library/Microsoft.Office.Interop.InfoPath.SemiTrust.Application.aspx) приложения. В проекте с управляемым кодом, созданном с помощью объектной модели, совместимой с InfoPath 2003, можно получить доступ к интерфейсу **XDocumentsCollection** с помощью переменной, созданной в методе кода формы  `thisApplication`  `_StartUp` проекта. В следующих строках кода создается переменная, которая указывает на интерфейс **XDocumentsCollection** текущего проекта. 
...
В проекте с управляемым кодом, созданном с помощью объектной модели, совместимой с InfoPath 2003, можно получить доступ к **интерфейсу XDocument** с помощью переменной, созданной в методе кода формы  `thisXDocument`  `StartUp` проекта. Следующая строка кода использует переменную для доступа к  `thisXDocument` **интерфейсу XDocument** текущего проекта для отображения URI открытой формы в предупреждении. 
...
> [!NOTE]
> Чтобы узнать больше о XML DOM и всех поддерживаемых свойствах и методах, см. MSXML SDK на сайте MSDN. 
