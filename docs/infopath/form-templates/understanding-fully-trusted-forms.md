...
InfoPath предоставляет возможность создания полностью доверенных форм, которые являются формами, которые имеют больше разрешений на безопасность и могут получать доступ к системным ресурсам и другим компонентам на компьютере пользователя. В этой статье описывается, что такое полностью доверенная форма и зачем она используется, а также создание полностью доверенной формы с помощью ручного преобразования и регистрации стандартной формы или цифровой подписи стандартной формы.
...
6. Добавьте следующие атрибуты в элемент **xDocumentClass** XSF-файла: 
   ...
   > [!NOTE]
   > [!Примечание] Значения, используемые для идентификатора URN могут быть любой строкой, если эта строка уникальна. После префикса должно быть по крайней мере два значения, и эти значения должны быть разделены `urn:` двоеточием. Кроме того, длина идентификатора URN не должна превышать 255 символов. 
