...
description: Microsoft InfoPath поддерживает перемещение шаблонов форм из одного расположения в другое, отправка их в виде вложения в сообщение электронной почты и создание шаблонов форм с полным доверием, которые имеют цифровую подпись или установлены.
...
Microsoft InfoPath поддерживает перемещение шаблонов форм из одного расположения в другое, отправка их в виде вложения в сообщение электронной почты и создание шаблонов форм с полным доверием, которые имеют цифровую подпись или установлены.
...
|**файл: путь\<\>доступа открыт из расположения или нет пути доступа (независимо от того, откуда получена форма)** <br/> |||||X  <br/> |
...
|:-----|:-----|:-----|:-----|
|**Доверенный (установленный или с доверенным сертификатом)** <br/> |Редактор открывается с уровнем "Полное доверие"  <br/> |Недоступно  <br/> |Недоступно  <br/> |
|**Уровень доверия "Домен": клиентский компьютер** <br/> |Не открывается  <br/> |Редактор откроется на уровне домена  <br/> |Редактор открывается с уровнем "Ограниченный"  <br/> |
|**Уровень доверия "Домен": интрасеть** <br/> |Не открывается  <br/> |Редактор откроется на уровне домена  <br/> |Редактор открывается с уровнем "Ограниченный"  <br/> |
|**Уровень доверия "Домен": Интернет** <br/> |Не открывается  <br/> |Редактор откроется на уровне домена  <br/> |Редактор открывается с уровнем "Ограниченный"  <br/> |
...
## <a name="mail-deployment-and-browser-enabled-form-templates"></a>Развертывание почты и шаблоны форм с поддержкой веб

InfoPath позволяет отправлять шаблоны форм в виде вложения в сообщение электронной почты и перемещать их из одного места в другое. Развертывание по почте  простой и эффективный способ распространения форм для использования внутри организации и доставки форм удаленным пользователям.
...
## <a name="understanding-form-identity"></a>Общие сведения об удостоверении формы
...
Все формы, создаваемые в конструкторе InfoPath, можно отправлять пользователям в виде вложений в сообщения электронной почты. Развертывание электронной почты — это простой и эффективный способ распространения форм для использования в офисе, а также для развертывания форм для удаленных пользователей.
  
### <a name="to-mail-a-form-template-to-other-users"></a>Отправка шаблона формы другим пользователям по электронной почте

1. Перейдите на вкладку **файл** , щелкните **опубликовать**, а затем нажмите кнопку **Электронная почта** . 
...
3. Отображается сообщение электронной почты, в котором вы можете заполнить список получателей и все необходимые для них инструкции.
    
4. Сделав это, нажмите кнопку **Отправить**. Форма и шаблон формы будут вложены в сообщение.
    
## <a name="email-deployment-restricted-domain-and-full-trust-form-templates"></a>Развертывание электронной почты: шаблоны форм с ограниченными возможностями, доменами и полным доверием

Развертывание ограниченных шаблонов форм в электронной почте позволяет открывать динамические формы без подключений к данным из любого места. Получатели могут открывать шаблоны форм, отправляемые как вложения электронной почты, непосредственно из Microsoft Outlook 2010 или из любого места, где получатель сохранил вложение. Кроме того, Outlook 2010 позволяет пользователям изменять формы непосредственно в сообщении.
  
Шаблоны форм с уровнем доверия "домен" должны быть открыты из их опубликованного расположения, но при публикации в списке получателей электронной почты в мастере публикации их можно отправить в виде вложения в сообщение электронной почты. При открытии вложения оно работает как ссылка на реальное место публикации шаблона. То, что действительно открывается в редакторе InfoPath,  это шаблон формы, находящийся в этом местоположении.
  
Использование шаблона формы уровня домена, отправляемого в виде вложения в сообщение электронной почты, похоже на любой другой тип документа; Например, книга Microsoft Excel или документ Microsoft Word. Пользователь может просто щелкнуть форму, после чего она откроется, и ее можно будет использовать. Кроме того, пользователям доступны все преимущества обновления на уровне домена.
  
Шаблоны форм, которые запрашивают доступ с полным доверием, можно отправлять по электронной почте, но эти шаблоны должны быть подписаны, иначе они не смогут открыться. Шаблоны форм, запрашивающие домен или ограниченный доступ, не обязательно должны быть подписаны как вложение электронной почты. InfoPath не проверяет подпись, даже если шаблон подписан, за исключением проверки на возможность автоматического обновления. При добавлении цифровой подписи к шаблону формы с уровнем доверия "Домен" или "Ограниченный" функция автоматического обновления будет по-прежнему доступна. В этом случае цифровая подпись заблокирует вывод сообщений о конфликте в кэше.
  
## <a name="sharing-forms-by-email-message-or-from-a-common-shared-location"></a>Совместное использование форм электронным сообщением или общим расположением

При создании формы, которая будет развернута в сообщении электронной почты, следует учитывать определенные вопросы.
...
### <a name="example-updating-domain-or-restricted-templates"></a>Пример: обновление доменных или ограниченных шаблонов
...
### <a name="example-deploying-restricted-form-templates-on-an-extranet"></a>Пример: развертывание шаблонов форм с ограниченным доступом в экстрасети
  
1. Сохраните шаблон формы домена на веб-сайте, на котором выполняется Microsoft SharePoint Foundation 2010.
...
8. Сохраните форму обратно на веб-сайт, на котором работает SharePoint Foundation 2010, и повторно свяжите форму с помощью параметра " **связать документы с этой библиотекой** " на странице " **Параметры библиотеки форм** ". 
    
## <a name="signature-verification-failure"></a>Ошибка проверки подписи
...
## <a name="infrastructure-registry-keys-for-form-migration-open-behavior"></a>Разделы реестра инфраструктуры для открытия режима переноса форм
...
|**Значение Name**|**Блок**|**Пользовательский интерфейс**|**Allow**|
|:-----|:-----|:-----|:-----|
|**Интернет** <br/> |X  <br/> |||
|**Интрасеть** <br/> ||X  <br/> ||
