...
Обработчики событий InfoPath следует создавать в конструкторе InfoPath, так как при использовании объектной модели, совместимой с InfoPath 2003, InfoPath автоматически добавляет правильное объявление и применяет атрибут ([инфопасевенсандлераттрибуте](https://msdn.microsoft.com/library/Microsoft.Office.Interop.InfoPath.SemiTrust.InfoPathEventHandlerAttribute.aspx) ) в файле кода формы (FormCode.cs или FormCode. vb) для идентификации и приемника обработчика событий. После создания обработчика событий не следует менять его объявление и атрибут в файле кода формы. 
